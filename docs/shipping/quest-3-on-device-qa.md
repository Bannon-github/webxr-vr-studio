...
14. **Present-path XR framebuffer scale clamp** (pre-headset authoring gate): immersive session uses `renderer.xr.setFramebufferScaleFactor(1)` (distinct from v0.15 `setPixelRatio(1)`). Three r170 has no getter and cannot rebuild the current eye buffer while presenting; the example also sets 1 at renderer setup so `setSession` inherits it. `sessionend` restores the saved lookdev/desktop scale. `crate-toolbox` v0.22. Not a measured ms. Raising above 1 is stretch-only.
