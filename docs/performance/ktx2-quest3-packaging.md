...
| Keep | Drop / never |
| --- | --- |
| `lod0` / `lod1` / `lod2` (or `userData.lodLevel` 0/1/2). Runtime (`crate-toolbox` v0.36) wires these into `userData.lod` and draws only one level. Missing names = all visuals stay visible | 4K PNG/JPEG, mipmaps-off; parenting `collider_*` or fastener under an LOD group |
