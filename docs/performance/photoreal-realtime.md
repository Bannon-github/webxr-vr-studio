...
**Recipe (Quest 3 props):** [ktx2-quest3-packaging.md](ktx2-quest3-packaging.md) — `gltf-transform` resize ≤1024² (LOD0 may be unlit/basic or `KHR_materials_unlit` with ≤256² albedo and no `normalTexture` / ORM; author LOD1 and LOD2 as unlit/basic or `KHR_materials_unlit` without a `baseColorTexture`), UASTC on remaining normal/ORM slots, ETC1S on albedo, then meshopt. Keep LOD nodes and `collider_*` off the hero mesh.
