...
**Recipe (Quest 3 props):** [ktx2-quest3-packaging.md](ktx2-quest3-packaging.md) — `gltf-transform` resize ≤1024² (prefer 512 on LOD0; author LOD1 as unlit/basic or `KHR_materials_unlit` with ≤256 albedo; author LOD2 as unlit/basic without a `baseColorTexture`), UASTC on normal/ORM, ETC1S on albedo, then meshopt. Keep LOD nodes and `collider_*` off the hero mesh.
