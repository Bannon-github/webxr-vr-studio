...
- **KTX2 / Basis** remains the packaging step when a DCC GLB lands (`docs/performance/ktx2-quest3-packaging.md`). Mid LOD primitives should omit `normalTexture` while keeping packed ORM. The runtime prefers a packaged GLB when present and does **not** strip `normalMap` at ingest. No GLB in this revision.
