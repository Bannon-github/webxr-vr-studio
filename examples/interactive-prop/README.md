...
| L3 LODs | LOD0 240 tris / 14 draws; LOD1 96 / 8; LOD2 24 / 2 (Three.js index counts). Auto switch 2.4 m / 4.5 m. LOD0, LOD1, and LOD2 are color-only unlit MeshBasic (no albedo map). Colliders are not LOD meshes. Packaged GLB: name groups `lod0` / `lod1` / `lod2` so ingest hides inactive levels (v0.36). |
