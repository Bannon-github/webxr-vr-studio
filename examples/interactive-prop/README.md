...
| --- | --- |
| Photoreal-ish look | Shared `MeshStandardMaterial`s + 512² procedural albedo/ORM/normal + IBL on LOD0 (full `normalScale`); LOD1 is albedo-only at 256² (no `normalMap`, no ORM; constant wood-ORM-midtone roughness/metalness on wood/handle, brass-ORM-midtone on latch); LOD2 is albedo-only at 256² (no `normalMap`, no ORM; constant wood-ORM-midtone roughness/metalness). Or a probed KTX2 GLB when present. Not 4K. |
...
| L3 LODs | LOD0 240 tris / 14 draws; LOD1 96 / 8; LOD2 24 / 2 (Three.js index counts). Auto switch 2.4 m / 4.5 m. LOD1 and LOD2 materials are albedo-only at 256² (omit `normalMap` and ORM). Colliders are not LOD meshes. |
