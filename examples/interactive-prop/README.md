...
| --- | --- |
| Photoreal-ish look | Shared color-only unlit `MeshBasicMaterial`s on LOD0 (wood midtone `0x633318` / brass midtone `0xBE7E31` / steel midtone `0xC1C3C9`; no map); LOD1 is color-only unlit `MeshBasicMaterial` (wood midtone `0x633318` / brass midtone `0xBE7E31`; no map); LOD2 is color-only unlit `MeshBasicMaterial` (wood midtone `0x633318`; no map). Or a probed KTX2 GLB when present. Not 4K. |
...
| L3 LODs | LOD0 240 tris / 6 draws / 230 unique verts / 4200 attrBytes; LOD1 96 / 4 / 100 / 1776; LOD2 24 / 2 / 48 / 720 (Three.js index counts; v0.37 same-material merge, was 14 / 8 / 2 draws; v0.39 coincident weld, was 440 / 192 / 48 verts; v0.40 unused uv/normal strip, was 8800 / 3776 / 1680 attrBytes). Auto switch 2.4 m / 4.5 m. LOD0, LOD1, and LOD2 are color-only unlit MeshBasic (no albedo map). Colliders are not LOD meshes. Packaged GLB: name groups `lod0` / `lod1` / `lod2` so ingest hides inactive levels (v0.36), merges same-material mesh children inside each group (v0.38), welds coincident vertices after concat (v0.39; prefer DCC pre-weld), and strips unused uv/normal on color-only MeshBasic (v0.40; author may omit those attrs in DCC). |
