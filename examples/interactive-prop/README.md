...
| L3 LODs | LOD0 240 tris / 6 draws; LOD1 96 / 4; LOD2 24 / 2 (Three.js index counts; v0.37 same-material merge, was 14 / 8 / 2). Auto switch 2.4 m / 4.5 m. LOD0, LOD1, and LOD2 are color-only unlit MeshBasic (no albedo map). Colliders are not LOD meshes. Packaged GLB: name groups `lod0` / `lod1` / `lod2` so ingest hides inactive levels (v0.36) and merges same-material mesh children inside each group (v0.38). |
