...
| L3 LODs | LOD0 240 tris / 6 draws / 230 unique verts; LOD1 96 / 4 / 100; LOD2 24 / 2 / 48 (Three.js index counts; v0.37 same-material merge, was 14 / 8 / 2 draws; v0.39 coincident weld, was 440 / 192 / 48 verts). Auto switch 2.4 m / 4.5 m. LOD0, LOD1, and LOD2 are color-only unlit MeshBasic (no albedo map). Colliders are not LOD meshes. Packaged GLB: name groups `lod0` / `lod1` / `lod2` so ingest hides inactive levels (v0.36), merges same-material mesh children inside each group (v0.38), and welds coincident vertices after concat (v0.39; prefer DCC pre-weld). |
