...
The example probes `/packaged/crate-toolbox.glb` (or `?packaged=`). Missing files must not look like a GLB (Vite may 200 HTML — the probe rejects `text/html`). **No file → procedural canvases (512² LOD0 MeshStandard; 256² LOD1 MeshStandard albedo-only; 256² LOD2 MeshBasic unlit).** The loader does not strip, downsample, or rewrite materials — author mid albedo at ≤256² and LOD2 as unlit/basic (or `KHR_materials_unlit`) with ≤256² albedo and no normal/ORM. Do not commit a fake or oversized binary.
