...
The example probes `/packaged/crate-toolbox.glb` (or `?packaged=`). Missing files must not look like a GLB (Vite may 200 HTML — the probe rejects `text/html`). **No file → procedural canvases (512² LOD0 MeshStandard; color-only LOD1 MeshBasic unlit; color-only LOD2 MeshBasic unlit).** The loader does not strip, downsample, or rewrite materials — author LOD1 and LOD2 as unlit/basic (or `KHR_materials_unlit`) **without** a `baseColorTexture` (or a tiny 1×1 / vertex color). LOD0 stays full PBR. Do not commit a fake or oversized binary.
