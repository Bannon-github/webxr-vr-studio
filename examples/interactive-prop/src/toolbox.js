 ...
 * Keeps `position` (and `color` when `vertexColors` is set). Includes
 * leftover skinning attributes via `stripUnusedColorOnlySkinAttributes`
 * (v0.89). Mapped or lit materials are left intact. Mutates in place.
 * Load-time only — author may omit these in DCC; this is a safety net
 * after merge/weld.
 */
export function stripUnusedColorOnlyAttributes(geometry, material) {
  if (!geometry || !isColorOnlyUnlitBasic(material)) return geometry;
  for (const name of COLOR_ONLY_UNUSED_ATTRS) {
    if (COLOR_ONLY_UNUSED_SKIN_ATTR_SET.has(name)) continue;
    if (geometry.getAttribute(name)) geometry.deleteAttribute(name);
  }
...
function isColliderMesh(mesh) {
  return Boolean(mesh?.userData?.collider || (mesh?.name && mesh.name.startsWith("collider_")));
}
 ...
 * Does not rewrite `updateRanges`. Does not change `usage`.
 * Does not replace attributes or the geometry. Does not delete
 ...
 * touch Object3D `animations`. Load-time only — not per-frame.
 */
 ...
 * unused `uv` / `normal` / `skinIndex` / `skinWeight` (and other
 * unused channels) when the material is color-only unlit MeshBasic
 * (v0.40; skinning attrs are the v0.89 extension of that strip),
 * then compact a
