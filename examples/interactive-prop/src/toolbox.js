...
/**
 * After leftover Material `index0AttributeName` is deleted, delete a
 * leftover own-property Material `defaultAttributeValues` so the r170
 * MeshBasic absence remains
 * (`material.defaultAttributeValues === undefined` and
 * `Object.hasOwn(material, 'defaultAttributeValues') === false`) on one
 * packed color-only unlit MeshBasic visual.
 *
 * Skip colliders. Skip interleaved geometries. Mapped / lit stay
 * authored. Prefer `delete material.defaultAttributeValues` when the
 * own property is present. Does not assign `null`, `undefined`, or
 * `{}`. Does not invent replacement defaults. Does not convert
 * MeshBasic to ShaderMaterial.
 *
 * An already-absent `defaultAttributeValues` is left alone (`delete`
 * is not called). Same `isColorOnlyUnlitBasic` gate as the
 * index0AttributeName pin.
 */
function pinColorOnlyMeshBasicMaterialDefaultAttributeValues(material) {
  if (!isColorOnlyUnlitBasic(material)) return;
  if (!Object.hasOwn(material, "defaultAttributeValues")) return;
  delete material.defaultAttributeValues;
}

/**
 * After leftover Material `index0AttributeName` is deleted (procedural
 * create / packaged detect), delete leftover Material
 * `defaultAttributeValues` on one packed color-only unlit MeshBasic
 * visual.
 *
 * Skip colliders. Skip interleaved geometries. Mapped / lit stay
 * authored. This helper deletes leftover `defaultAttributeValues` on
 * color-only stand-ins — that is the pulse, so a color-only material
 * is not skipped merely because it still has an own
 * `defaultAttributeValues` object. Does not invent replacement
 * defaults. Does not assign `null`, `undefined`, or `{}`. Does not
 * convert MeshBasic to ShaderMaterial.
 *
 * An already-absent `defaultAttributeValues` is left as-is.
 *
 * Same `isColorOnlyUnlitBasic` gate as the index0AttributeName pin.
 */
export function pinColorOnlyUnlitBasicMaterialDefaultAttributeValues(mesh) {
  if (!mesh?.isMesh) return mesh;
  if (isColliderMesh(mesh)) return mesh;
  if (!isColorOnlyUnlitBasic(mesh.material)) return mesh;
  if (!mesh.geometry) return mesh;
  if (colorOnlyGeometryInterleaved(mesh.geometry)) return mesh;
  pinColorOnlyMeshBasicMaterialDefaultAttributeValues(mesh.material);
  return mesh;
}
 ...
 * After leftover Material `index0AttributeName` is deleted (procedural
 * create / packaged detect), delete leftover Material
 * `defaultAttributeValues` so the r170 MeshBasic absence remains
 * (`material.defaultAttributeValues === undefined` and
 * `Object.hasOwn(material, 'defaultAttributeValues') === false`) on
 * every packed color-only unlit MeshBasic visual material (shared wood
 * / brass / steel stand-ins; body LOD leaves + lid/latch/tool +
 * fastener; unique MeshBasic stays 3). Measured
 * defaultAttributeValues-absent 3.
 *
 * Skip colliders (even MeshBasic debug hulls). Skip mapped / lit.
 * Skip interleaved geometries (same pack-pipeline interleaved gate).
 * Skip meshes whose material is shared with a blocked collider or
 * interleaved mesh so that shared material keeps its authored
 * `defaultAttributeValues`. Skip geometries shared with a collider, an
 * interleaved mesh, or a mapped / lit mesh so a visual that shares
 * that geometry does not pin its material. Prefer
 ...
 * Pin once per shared material instance. Does not assign `null`,
 * `undefined`, or `{}`. Does not invent replacement defaults. Does
 * not convert MeshBasic to ShaderMaterial. Does not touch Material
 * `index0AttributeName` (v1.13.0 index0AttributeName-absent stays).
 * Does not touch Material `depthPacking` (v1.12.0 depthPacking-absent
 * stays). Does not touch Material `extensions` (v1.11.0
 * extensions-absent stays). Does not touch BufferGeometry `indirect`
 * (v1.10.0 indirect-null stays). Does not re-run the unused-channel
 ...
 * **Checked installed three@0.170.0:** fresh `Material` /
 * `MeshBasicMaterial` constructors do not assign
 * `defaultAttributeValues` (`defaultAttributeValues === undefined` and
 * `Object.hasOwn` is false). `ShaderMaterial` assigns
 * `this.defaultAttributeValues = { color: [1, 1, 1], uv: [0, 0], uv1: [0, 0] }`.
 * `ShaderMaterial.copy` does not copy `defaultAttributeValues`; the
 * copy keeps the map its constructor assigned (not a clone of the
 * source object). `WebGLPrograms` does not read
 * `defaultAttributeValues` (no cache-key token and no program-layer
 * bit). `WebGLProgram` emits no `#define` for it.
 * `WebGLBindingStates.setupVertexAttributes` reads
 ...
 * it looks up `materialDefaultAttributeValues[name]` and, when that
 * value is not `undefined`, calls `gl.vertexAttrib2fv` /
 * `gl.vertexAttrib3fv` / `gl.vertexAttrib4fv` / `gl.vertexAttrib1fv`
 * from `value.length`. A leftover ShaderMaterial-style map forces
 * those constant vertexAttrib uploads for missing channels (for
 * example after the color-only unused-channel strip) while the
 * program cache key stays identical to constructor absence. An empty
 * `{}` is not `undefined`, so the lookup branch is entered even when
 * no name matches. Assigning `null` also enters that branch
 * (`null !== undefined`) and then throws on the property lookup.
 * Assigning `undefined` skips the branch but, stored as an own
 * property, is not the r170 MeshBasic absence.
 * `delete material.defaultAttributeValues` removes the own property
 * so the r170 absence remains. On clean procedural materials
 * (constructor absence already) draws, tris, and attrBytes stay
 * unchanged vs v1.13.0. Load-time only — not per-frame. No invented
 * headset ms.
 */
export function pinColorOnlyVisualMaterialDefaultAttributeValues(entity) {
  if (!entity) return entity;
  const blockedMaterials = new Set();
  const blockedGeometries = new Set();
  const meshes = [];
  ...
  for (const mesh of meshes) {
    ...
    if (seenMaterials.has(mesh.material)) continue;
    seenMaterials.add(mesh.material);
    pinColorOnlyUnlitBasicMaterialDefaultAttributeValues(mesh);
  }
  return entity;
}

/**
 * After leftover Material `defaultAttributeValues` is deleted, delete a
 * leftover own-property Material `uniforms` so the r170 MeshBasic
 * absence remains (`material.uniforms === undefined` and
 * `Object.hasOwn(material, 'uniforms') === false`) on one packed
 * color-only unlit MeshBasic visual.
 *
 * Skip colliders. Skip interleaved geometries. Mapped / lit stay
 * authored. Prefer `delete material.uniforms` when the own property
 * is present. Does not assign `null`, `undefined`, or `{}`. Does not
 * invent replacement uniforms. Does not convert MeshBasic to
 * ShaderMaterial. Does not clear `uniforms` on real ShaderMaterial /
 * RawShaderMaterial (those fail `isColorOnlyUnlitBasic`).
 *
 * An already-absent `uniforms` is left alone (`delete` is not called).
 * Same `isColorOnlyUnlitBasic` gate as the defaultAttributeValues pin.
 */
function pinColorOnlyMeshBasicMaterialUniforms(material) {
  if (!isColorOnlyUnlitBasic(material)) return;
  if (!Object.hasOwn(material, "uniforms")) return;
  delete material.uniforms;
}

/**
 * After leftover Material `defaultAttributeValues` is deleted
 * (procedural create / packaged detect), delete leftover Material
 * `uniforms` on one packed color-only unlit MeshBasic visual.
 *
 * Skip colliders. Skip interleaved geometries. Mapped / lit stay
 * authored. This helper deletes leftover `uniforms` on color-only
 * stand-ins — that is the pulse, so a color-only material is not
 * skipped merely because it still has an own `uniforms` object. Does
 * not invent replacement uniforms. Does not assign `null`,
 * `undefined`, or `{}`. Does not convert MeshBasic to ShaderMaterial.
 * Does not clear `uniforms` on real ShaderMaterial / RawShaderMaterial.
 *
 * An already-absent `uniforms` is left as-is.
 *
 * Same `isColorOnlyUnlitBasic` gate as the defaultAttributeValues pin.
 */
export function pinColorOnlyUnlitBasicMaterialUniforms(mesh) {
  if (!mesh?.isMesh) return mesh;
  if (isColliderMesh(mesh)) return mesh;
  if (!isColorOnlyUnlitBasic(mesh.material)) return mesh;
  if (!mesh.geometry) return mesh;
  if (colorOnlyGeometryInterleaved(mesh.geometry)) return mesh;
  pinColorOnlyMeshBasicMaterialUniforms(mesh.material);
  return mesh;
}
 ...
 * `Object.hasOwn(material, 'uniforms') === false`) on every packed
 * color-only unlit MeshBasic visual material (shared wood / brass /
 * steel stand-ins; body LOD leaves + lid/latch/tool + fastener; unique
 * MeshBasic stays 3). Measured uniforms-absent 3.
 *
 * Skip colliders (even MeshBasic debug hulls). Skip mapped / lit.
 * Skip interleaved geometries (same pack-pipeline interleaved gate).
 * Skip meshes whose material is shared with a blocked collider or
 * interleaved mesh so that shared material keeps its authored
 * `uniforms`. Skip geometries shared with a collider, an interleaved
 * mesh, or a mapped / lit mesh so a visual that shares that geometry
 * does not pin its material. Prefer `delete material.uniforms` when
 * the own property is present. An already-absent `uniforms` is left
 * alone. Pin once per shared material instance. Does not assign
 * `null`, `undefined`, or `{}`. Does not invent replacement uniforms.
 * Does not convert MeshBasic to ShaderMaterial. Does not clear
 * `uniforms` on real ShaderMaterial / RawShaderMaterial. Does not
 * touch Material `defaultAttributeValues` (v1.14.0
 * defaultAttributeValues-absent stays). Does not touch Material
 * `index0AttributeName` (v1.13.0 index0AttributeName-absent stays).
 * Does not touch Material `depthPacking` (v1.12.0 depthPacking-absent
 * stays). Does not touch Material `extensions` (v1.11.0
 * extensions-absent stays). Does not touch BufferGeometry `indirect`
 ...
 * BufferGeometry `userData` / `name`. Does not touch BufferAttribute
 * fields. Does not touch prior material program-cache / flag pins.
 * Does not touch `uniformsGroups`, `uniformsNeedUpdate`,
 * `vertexShader`, `fragmentShader`, `lights`, `clipping`, or
 * `isShaderMaterial`. Does not touch bounds, morphs, animations,
 * shadows, `frustumCulled`, or `mesh.visible`.
 *
 * **Checked installed three@0.170.0:** fresh `Material` /
 * `MeshBasicMaterial` constructors do not assign `uniforms`
 * (`uniforms === undefined` and `Object.hasOwn` is false).
 ...
 * MeshBasic absence. `delete material.uniforms` removes the own
 * property so the r170 absence remains. On clean procedural materials
 * (constructor absence already) draws, tris, and attrBytes stay
 * unchanged vs v1.14.0. Load-time only — not per-frame. No invented
 * headset ms.
 */
export function pinColorOnlyVisualMaterialUniforms(entity) {
  if (!entity) return entity;
  const blockedMaterials = new Set();
  const blockedGeometries = new Set();
  const meshes = [];
  ...
  for (const mesh of meshes) {
    ...
    if (seenMaterials.has(mesh.material)) continue;
    seenMaterials.add(mesh.material);
    pinColorOnlyUnlitBasicMaterialUniforms(mesh);
  }
  return entity;
}

/**
 * After leftover Material `uniforms` is deleted, delete a leftover
 * own-property Material `uniformsNeedUpdate` so the r170 MeshBasic
 * absence remains (`material.uniformsNeedUpdate === undefined` and
 * `Object.hasOwn(material, 'uniformsNeedUpdate') === false`) on one
 * packed color-only unlit MeshBasic visual.
 *
 * Skip colliders. Skip interleaved geometries. Mapped / lit stay
 * authored. Prefer `delete material.uniformsNeedUpdate` when the own
 * property is present. Does not assign `null`, `undefined`, `false`,
 * or `true`. Does not invent a custom shader path. Does not convert
 * MeshBasic to ShaderMaterial. Does not clear `uniformsNeedUpdate` on
 * real ShaderMaterial / RawShaderMaterial (those fail
 * `isColorOnlyUnlitBasic`).
 *
 * An already-absent `uniformsNeedUpdate` is left alone (`delete` is
 * not called). Same `isColorOnlyUnlitBasic` gate as the uniforms pin.
 */
function pinColorOnlyMeshBasicMaterialUniformsNeedUpdate(material) {
  if (!isColorOnlyUnlitBasic(material)) return;
  if (!Object.hasOwn(material, "uniformsNeedUpdate")) return;
  delete material.uniformsNeedUpdate;
}

/**
 * After leftover Material `uniforms` is deleted (procedural create /
 * packaged detect), delete leftover Material `uniformsNeedUpdate` on
 * one packed color-only unlit MeshBasic visual.
 *
 * Skip colliders. Skip interleaved geometries. Mapped / lit stay
 * authored. This helper deletes leftover `uniformsNeedUpdate` on
 * color-only stand-ins — that is the pulse, so a color-only material
 * is not skipped merely because it still has an own
 * `uniformsNeedUpdate`. Does not assign `null`, `undefined`, `false`,
 * or `true`. Does not invent a custom shader path. Does not convert
 * MeshBasic to ShaderMaterial. Does not clear `uniformsNeedUpdate` on
 * real ShaderMaterial / RawShaderMaterial.
 *
 * An already-absent `uniformsNeedUpdate` is left as-is.
 *
 * Same `isColorOnlyUnlitBasic` gate as the uniforms pin.
 */
export function pinColorOnlyUnlitBasicMaterialUniformsNeedUpdate(mesh) {
  if (!mesh?.isMesh) return mesh;
  if (isColliderMesh(mesh)) return mesh;
  if (!isColorOnlyUnlitBasic(mesh.material)) return mesh;
  if (!mesh.geometry) return mesh;
  if (colorOnlyGeometryInterleaved(mesh.geometry)) return mesh;
  pinColorOnlyMeshBasicMaterialUniformsNeedUpdate(mesh.material);
  return mesh;
}

/**
 * After leftover Material `uniforms` is deleted (procedural create /
 * packaged detect), delete leftover Material `uniformsNeedUpdate` so
 * the r170 MeshBasic absence remains
 * (`material.uniformsNeedUpdate === undefined` and
 * `Object.hasOwn(material, 'uniformsNeedUpdate') === false`) on every
 * packed color-only unlit MeshBasic visual material (shared wood /
 * brass / steel stand-ins; body LOD leaves + lid/latch/tool +
 * fastener; unique MeshBasic stays 3). Measured
 * uniformsNeedUpdate-absent 3.
 *
 * Skip colliders (even MeshBasic debug hulls). Skip mapped / lit.
 * Skip interleaved geometries (same pack-pipeline interleaved gate).
 * Skip meshes whose material is shared with a blocked collider or
 * interleaved mesh so that shared material keeps its authored
 * `uniformsNeedUpdate`. Skip geometries shared with a collider, an
 * interleaved mesh, or a mapped / lit mesh so a visual that shares
 * that geometry does not pin its material. Prefer
 ...
 * once per shared material instance. Does not assign `null`,
 * `undefined`, `false`, or `true`. Does not invent a custom shader
 * path. Does not convert MeshBasic to ShaderMaterial. Does not clear
 * `uniformsNeedUpdate` on real ShaderMaterial / RawShaderMaterial.
 * Does not touch Material `uniforms` (v1.15.0 uniforms-absent stays).
 * Does not touch Material `defaultAttributeValues` (v1.14.0
 * defaultAttributeValues-absent stays). Does not touch Material
 * `index0AttributeName` (v1.13.0 index0AttributeName-absent stays).
 * Does not touch Material `depthPacking` (v1.12.0 depthPacking-absent
 * stays). Does not touch Material `extensions` (v1.11.0
 ...
 * BufferGeometry `userData` / `name`. Does not touch BufferAttribute
 * fields. Does not touch prior material program-cache / flag pins.
 * Does not touch `uniformsGroups`, `vertexShader`, `fragmentShader`,
 * `lights`, `clipping`, or `isShaderMaterial`. Does not touch bounds,
 * morphs, animations, shadows, `frustumCulled`, or `mesh.visible`.
 *
 * **Checked installed three@0.170.0:** fresh `Material` /
 * `MeshBasicMaterial` constructors do not assign `uniformsNeedUpdate`
 * (`uniformsNeedUpdate === undefined` and `Object.hasOwn` is false).
 ...
 * absence. `delete material.uniformsNeedUpdate` removes the own
 * property so the r170 absence remains. On clean procedural materials
 * (constructor absence already) draws, tris, and attrBytes stay
 * unchanged vs v1.15.0. Load-time only — not per-frame. No invented
 * headset ms.
 */
export function pinColorOnlyVisualMaterialUniformsNeedUpdate(entity) {
  if (!entity) return entity;
  const blockedMaterials = new Set();
  const blockedGeometries = new Set();
  const meshes = [];
  ...
  for (const mesh of meshes) {
    ...
    if (seenMaterials.has(mesh.material)) continue;
    seenMaterials.add(mesh.material);
    pinColorOnlyUnlitBasicMaterialUniformsNeedUpdate(mesh);
  }
  return entity;
}

/**
 * After leftover Material `uniformsNeedUpdate` is deleted, delete a
 * leftover own-property Material `uniformsGroups` so the r170 MeshBasic
 * absence remains (`material.uniformsGroups === undefined` and
 * `Object.hasOwn(material, 'uniformsGroups') === false`) on one packed
 * color-only unlit MeshBasic visual.
 *
 * Skip colliders. Skip interleaved geometries. Mapped / lit stay
 * authored. Prefer `delete material.uniformsGroups` when the own
 * property is present. Does not assign `null`, `undefined`, `[]`, or
 * `{}`. Does not invent a custom shader path. Does not convert
 * MeshBasic to ShaderMaterial. Does not clear `uniformsGroups` on real
 * ShaderMaterial / RawShaderMaterial (those fail
 * `isColorOnlyUnlitBasic`).
 *
 * An already-absent `uniformsGroups` is left alone (`delete` is not
 * called). Same `isColorOnlyUnlitBasic` gate as the uniformsNeedUpdate
 * pin.
 */
function pinColorOnlyMeshBasicMaterialUniformsGroups(material) {
  if (!isColorOnlyUnlitBasic(material)) return;
  if (!Object.hasOwn(material, "uniformsGroups")) return;
  delete material.uniformsGroups;
}

/**
 * After leftover Material `uniformsNeedUpdate` is deleted (procedural
 * create / packaged detect), delete leftover Material `uniformsGroups`
 * on one packed color-only unlit MeshBasic visual.
 *
 * Skip colliders. Skip interleaved geometries. Mapped / lit stay
 * authored. This helper deletes leftover `uniformsGroups` on
 * color-only stand-ins — that is the pulse, so a color-only material
 * is not skipped merely because it still has an own `uniformsGroups`.
 * Does not assign `null`, `undefined`, `[]`, or `{}`. Does not invent
 * a custom shader path. Does not convert MeshBasic to ShaderMaterial.
 * Does not clear `uniformsGroups` on real ShaderMaterial /
 * RawShaderMaterial.
 *
 * An already-absent `uniformsGroups` is left as-is.
 *
 * Same `isColorOnlyUnlitBasic` gate as the uniformsNeedUpdate pin.
 */
export function pinColorOnlyUnlitBasicMaterialUniformsGroups(mesh) {
  if (!mesh?.isMesh) return mesh;
  if (isColliderMesh(mesh)) return mesh;
  if (!isColorOnlyUnlitBasic(mesh.material)) return mesh;
  if (!mesh.geometry) return mesh;
  if (colorOnlyGeometryInterleaved(mesh.geometry)) return mesh;
  pinColorOnlyMeshBasicMaterialUniformsGroups(mesh.material);
  return mesh;
}
 ...
 * `Object.hasOwn(material, 'uniformsGroups') === false`) on every
 * packed color-only unlit MeshBasic visual material (shared wood /
 * brass / steel stand-ins; body LOD leaves + lid/latch/tool +
 * fastener; unique MeshBasic stays 3). Measured uniformsGroups-absent
 * 3.
 *
 * Skip colliders (even MeshBasic debug hulls). Skip mapped / lit.
 * Skip interleaved geometries (same pack-pipeline interleaved gate).
 * Skip meshes whose material is shared with a blocked collider or
 * interleaved mesh so that shared material keeps its authored
 * `uniformsGroups`. Skip geometries shared with a collider, an
 * interleaved mesh, or a mapped / lit mesh so a visual that shares
 * that geometry does not pin its material. Prefer
 * `delete material.uniformsGroups` when the own property is present.
 * An already-absent `uniformsGroups` is left alone. Pin once per
 * shared material instance. Does not assign `null`, `undefined`,
 * `[]`, or `{}`. Does not invent a custom shader path. Does not
 * convert MeshBasic to ShaderMaterial. Does not clear `uniformsGroups`
 * on real ShaderMaterial / RawShaderMaterial. Does not touch Material
 * `uniformsNeedUpdate` (v1.16.0 uniformsNeedUpdate-absent stays). Does
 * not touch Material `uniforms` (v1.15.0 uniforms-absent stays). Does
 * not touch Material `defaultAttributeValues` (v1.14.0
 * defaultAttributeValues-absent stays). Does not touch Material
 * `index0AttributeName` (v1.13.0 index0AttributeName-absent stays).
 * Does not touch Material `depthPacking` (v1.12.0 depthPacking-absent
 * stays). Does not touch Material `extensions` (v1.11.0
 ...
 * BufferGeometry `userData` / `name`. Does not touch BufferAttribute
 * fields. Does not touch prior material program-cache / flag pins.
 * Does not touch `vertexShader`, `fragmentShader`, `lights`,
 * `clipping`, or `isShaderMaterial`. Does not touch bounds, morphs,
 * animations, shadows, `frustumCulled`, or `mesh.visible`.
 *
 * **Checked installed three@0.170.0:** fresh `Material` /
 * `MeshBasicMaterial` constructors do not assign `uniformsGroups`
 * (`uniformsGroups === undefined` and `Object.hasOwn` is false).
 ...
 * MeshBasic is neither, so a leftover array, empty array, `null`, or
 * `{}` is not uploaded as uniform buffer objects and does not change
 * the draw. Assigning `null`, `undefined`, `[]`, or `{}` stores an
 * own property, which is not the r170 MeshBasic absence.
 * `delete material.uniformsGroups` removes the own property so the
 * r170 absence remains. On clean procedural materials (constructor
 * absence already) draws, tris, and attrBytes stay unchanged vs
 * v1.16.0. Load-time only — not per-frame. No invented headset ms.
 */
export function pinColorOnlyVisualMaterialUniformsGroups(entity) {
  if (!entity) return entity;
  const blockedMaterials = new Set();
  const blockedGeometries = new Set();
  const meshes = [];
  ...
  for (const mesh of meshes) {
    ...
    if (seenMaterials.has(mesh.material)) continue;
    seenMaterials.add(mesh.material);
    pinColorOnlyUnlitBasicMaterialUniformsGroups(mesh);
  }
  return entity;
}

/**
 * After leftover Material `uniformsGroups` is deleted, delete a
 * leftover own-property Material `vertexShader` so the r170 MeshBasic
 * absence remains (`material.vertexShader === undefined` and
 * `Object.hasOwn(material, 'vertexShader') === false`) on one packed
 * color-only unlit MeshBasic visual.
 *
 * Skip colliders. Skip interleaved geometries. Mapped / lit stay
 * authored. Prefer `delete material.vertexShader` when the own
 * property is present. Does not assign `null`, `undefined`, `''`, or
 * a stub GLSL string. Does not invent a custom shader path. Does not
 * convert MeshBasic to ShaderMaterial. Does not clear `vertexShader`
 * on real ShaderMaterial / RawShaderMaterial (those fail
 * `isColorOnlyUnlitBasic`).
 *
 * An already-absent `vertexShader` is left alone (`delete` is not
 * called). Same `isColorOnlyUnlitBasic` gate as the uniformsGroups
 * pin.
 */
function pinColorOnlyMeshBasicMaterialVertexShader(material) {
  if (!isColorOnlyUnlitBasic(material)) return;
  if (!Object.hasOwn(material, "vertexShader")) return;
  delete material.vertexShader;
}

/**
 * After leftover Material `uniformsGroups` is deleted (procedural
 * create / packaged detect), delete leftover Material `vertexShader`
 * on one packed color-only unlit MeshBasic visual.
 *
 * Skip colliders. Skip interleaved geometries. Mapped / lit stay
 * authored. This helper deletes leftover `vertexShader` on
 * color-only stand-ins — that is the pulse, so a color-only material
 * is not skipped merely because it still has an own `vertexShader`.
 * Does not assign `null`, `undefined`, `''`, or a stub GLSL string.
 * Does not invent a custom shader path. Does not convert MeshBasic to
 * ShaderMaterial. Does not clear `vertexShader` on real
 * ShaderMaterial / RawShaderMaterial.
 *
 * An already-absent `vertexShader` is left as-is.
 *
 * Same `isColorOnlyUnlitBasic` gate as the uniformsGroups pin.
 */
export function pinColorOnlyUnlitBasicMaterialVertexShader(mesh) {
  if (!mesh?.isMesh) return mesh;
  if (isColliderMesh(mesh)) return mesh;
  if (!isColorOnlyUnlitBasic(mesh.material)) return mesh;
  if (!mesh.geometry) return mesh;
  if (colorOnlyGeometryInterleaved(mesh.geometry)) return mesh;
  pinColorOnlyMeshBasicMaterialVertexShader(mesh.material);
  return mesh;
}
 ...
 * `Object.hasOwn(material, 'vertexShader') === false`) on every
 * packed color-only unlit MeshBasic visual material (shared wood /
 * brass / steel stand-ins; body LOD leaves + lid/latch/tool +
 * fastener; unique MeshBasic stays 3). Measured vertexShader-absent
 * 3.
 *
 * Skip colliders (even MeshBasic debug hulls). Skip mapped / lit.
 * Skip interleaved geometries (same pack-pipeline interleaved gate).
 * Skip meshes whose material is shared with a blocked collider or
 * interleaved mesh so that shared material keeps its authored
 * `vertexShader`. Skip geometries shared with a collider, an
 * interleaved mesh, or a mapped / lit mesh so a visual that shares
 * that geometry does not pin its material. Prefer
 * `delete material.vertexShader` when the own property is present.
 * An already-absent `vertexShader` is left alone. Pin once per
 * shared material instance. Does not assign `null`, `undefined`,
 * `''`, or a stub GLSL string. Does not invent a custom shader path.
 * Does not convert MeshBasic to ShaderMaterial. Does not clear
 * `vertexShader` on real ShaderMaterial / RawShaderMaterial. Does not
 * touch Material `uniformsGroups` (v1.17.0 uniformsGroups-absent
 * stays). Does not touch Material `uniformsNeedUpdate` (v1.16.0
 * uniformsNeedUpdate-absent stays). Does not touch Material `uniforms`
 * (v1.15.0 uniforms-absent stays). Does not touch Material
 * `defaultAttributeValues` (v1.14.0 defaultAttributeValues-absent
 * stays). Does not touch Material `index0AttributeName` (v1.13.0
 * index0AttributeName-absent stays). Does not touch Material
 * `depthPacking` (v1.12.0 depthPacking-absent stays). Does not touch
 * Material `extensions` (v1.11.0 extensions-absent stays). Does not
 * touch BufferGeometry `indirect` (v1.10.0 indirect-null stays). Does
 * not re-run the unused-channel strip (v1.9.0 unusedAttributes-absent
 * stays). Does not touch `onUpload` / `onUploadCallback` (v1.8.0
 * onUpload-release stays). Does not delete `color` (v1.7.0
 * colorAttribute-absent stays). Does not touch Material `version`
 ...
 * BufferGeometry `userData` / `name`. Does not touch BufferAttribute
 * fields. Does not touch prior material program-cache / flag pins.
 * Does not touch `fragmentShader`, `lights`, `clipping`, or
 * `isShaderMaterial`. Does not touch bounds, morphs, animations,
 * shadows, `frustumCulled`, or `mesh.visible`.
 *
 * **Checked installed three@0.170.0:** fresh `Material` /
 * `MeshBasicMaterial` constructors do not assign `vertexShader`
 * (`vertexShader === undefined` and `Object.hasOwn` is false).
 ...
 * `getProgramCacheKey` pushes `parameters.shaderID` when present, so
 * a leftover `material.vertexShader` is not a cache-key token for
 ...
 * `''`, or a stub GLSL string stores an own property, which is not
 * the r170 MeshBasic absence. `delete material.vertexShader` removes
 * the own property so the r170 absence remains. On clean procedural
 * materials (constructor absence already) draws, tris, and attrBytes
 * stay unchanged vs v1.17.0. Load-time only — not per-frame. No
 * invented headset ms.
 */
export function pinColorOnlyVisualMaterialVertexShader(entity) {
  if (!entity) return entity;
  const blockedMaterials = new Set();
  const blockedGeometries = new Set();
  const meshes = [];
  ...
  for (const mesh of meshes) {
    ...
    if (seenMaterials.has(mesh.material)) continue;
    seenMaterials.add(mesh.material);
    pinColorOnlyUnlitBasicMaterialVertexShader(mesh);
  }
  return entity;
}
...
export function createToolbox() {
  ...
  // count.
  // v1.14.0: after that index0AttributeName pin, delete leftover
  ...
  // measured defaultAttributeValues-absent 3).
  // pinColorOnlyUnlitBasicMaterialDefaultAttributeValues /
  // pinColorOnlyVisualMaterialDefaultAttributeValues run after
  // pinColorOnlyVisualMaterialIndex0AttributeName.
  // Prefer delete material.defaultAttributeValues when the own
  // property is present. Do not assign null, undefined, or {}. An
  // already-absent defaultAttributeValues is left alone. Pin once per
  // shared material instance. Do not invent replacement defaults. Do
  // not convert MeshBasic to ShaderMaterial. Do not touch
  // index0AttributeName (v1.13.0 index0AttributeName-absent 3 stays).
  // Do not touch depthPacking (v1.12.0 depthPacking-absent 3 stays).
  // Do not touch extensions (v1.11.0 extensions-absent 3 stays). Do
  // not touch indirect (v1.10.0 indirect-null 13 stays). Do not re-run
  // the unused-channel strip (v1.9.0 unusedAttributes-absent 13 stays).
  // Do not touch onUpload / onUploadCallback (v1.8.0 onUpload-release
  // 13 stays). Do not touch color (v1.7.0 colorAttribute-absent 13
  // stays). Do not touch matrixWorldNeedsUpdate (v1.6.0 stays). Do not
  // change matrixAutoUpdate or matrixWorldAutoUpdate. Do not touch
  // Material version / name / userData or Mesh name / userData or
  // BufferGeometry name / userData. Do not touch BufferAttribute
  // fields. Do not touch prior Material program-cache / flag pins.
  // Do not touch bounds, morphs, animations, shadows, frustumCulled,
  // or mesh.visible.
  ...
  // this.defaultAttributeValues = { color: [1, 1, 1], uv: [0, 0], uv1: [0, 0] }.
  // ShaderMaterial.copy does not copy defaultAttributeValues; the copy
  // keeps the map its constructor assigned (not a clone of the source
  // object). WebGLPrograms does not read defaultAttributeValues (no
  // cache-key token and no program-layer bit). WebGLProgram emits no
  // #define for it. WebGLBindingStates.setupVertexAttributes reads
  // material.defaultAttributeValues. When a program attribute has no
  // geometry attribute and materialDefaultAttributeValues !== undefined,
  // it looks up materialDefaultAttributeValues[name] and may call
  // gl.vertexAttrib2fv / 3fv / 4fv / 1fv with that value. A leftover
  // ShaderMaterial-style map forces those constant vertexAttrib uploads
  // for missing channels while the program cache key stays identical
  // to constructor absence. An empty {} still enters the lookup
  // ({} !== undefined). Assigning null also enters it and then throws
  // on the property lookup. Deleting the own property restores the
  // r170 absence. On clean procedural materials draws / tris /
  // attrBytes stay unchanged vs v1.13.0. No invented headset ms.
  // Draws / tris / attrBytes unchanged vs v1.13.0 on clean procedural
  // meshes. index0AttributeName-absent 3 stays. depthPacking-absent 3
  // stays. extensions-absent 3 stays. indirect-null 13 stays.
  // unusedAttributes-absent 13 stays. onUpload-release 13 stays.
  // colorAttribute-absent 13 stays. matrixWorldNeedsUpdate-false 13
  // stays. material-version-zero 3 stays. mesh-userData-empty 13
  // stays. mesh-name-empty 10 stays. unique MeshBasic 3 stays.
  // defaultAttributeValues-absent 3 is the new count.
  // v1.15.0: after that defaultAttributeValues pin, delete leftover
  // Material uniforms so the r170 MeshBasic absence remains
  // (material.uniforms === undefined and
  // Object.hasOwn(material, 'uniforms') === false) on the 3 shared
  // color-only MeshBasic materials (wood / brass / steel; measured
  // uniforms-absent 3).
  // pinColorOnlyUnlitBasicMaterialUniforms /
  // pinColorOnlyVisualMaterialUniforms run after
  // pinColorOnlyVisualMaterialDefaultAttributeValues.
  // Prefer delete material.uniforms when the own property is present.
  // Do not assign null, undefined, or {}. An already-absent uniforms
  // is left alone. Pin once per shared material instance. Do not
  ...
  // defaultAttributeValues-absent 3 stays). Do not touch
  // index0AttributeName (v1.13.0 index0AttributeName-absent 3 stays).
  // Do not touch depthPacking (v1.12.0 depthPacking-absent 3 stays).
  // Do not touch extensions (v1.11.0 extensions-absent 3 stays). Do
  // not touch indirect (v1.10.0 indirect-null 13 stays). Do not re-run
  ...
  // BufferGeometry name / userData. Do not touch BufferAttribute
  // fields. Do not touch prior Material program-cache / flag pins.
  // Do not touch uniformsGroups, uniformsNeedUpdate, vertexShader,
  // fragmentShader, lights, clipping, or isShaderMaterial. Do not
  // touch bounds, morphs, animations, shadows, frustumCulled, or
  // mesh.visible.
  // Checked installed three@0.170.0:
  // Fresh Material / MeshBasicMaterial constructors do not assign
  // uniforms. ShaderMaterial assigns this.uniforms = {} and copy
  // clones uniforms via cloneUniforms(source.uniforms).
  // RawShaderMaterial extends ShaderMaterial, so it keeps that
  // constructor uniforms object. WebGLPrograms.getUniforms uses
  // material.uniforms only when shaderIDs[material.type] is absent
  // (custom ShaderMaterial / RawShaderMaterial path). MeshBasic
  // resolves shaderID 'basic' and clones ShaderLib uniforms instead,
  // so a leftover uniforms object is not the active upload map — but
  // it still retains heap (Texture / Vector / Color uniform values)
  // and can confuse ingest/debug when DCC or mistaken ShaderMaterial
  // assignment bleeds onto color-only MeshBasic. Assigning null,
  // undefined, or {} stores an own property, which is not the r170
  // MeshBasic absence. Deleting the own property restores the r170
  // absence. On clean procedural materials draws / tris / attrBytes
  // stay unchanged vs v1.14.0. No invented headset ms.
  // Draws / tris / attrBytes unchanged vs v1.14.0 on clean procedural
  // meshes. defaultAttributeValues-absent 3 stays.
  // index0AttributeName-absent 3 stays. depthPacking-absent 3 stays.
  // extensions-absent 3 stays. indirect-null 13 stays.
  // unusedAttributes-absent 13 stays. onUpload-release 13 stays.
  // colorAttribute-absent 13 stays. matrixWorldNeedsUpdate-false 13
  // stays. material-version-zero 3 stays. mesh-userData-empty 13
  // stays. mesh-name-empty 10 stays. unique MeshBasic 3 stays.
  // uniforms-absent 3 is the new count.
  // v1.16.0: after that uniforms pin, delete leftover Material
  // uniformsNeedUpdate so the r170 MeshBasic absence remains
  // (material.uniformsNeedUpdate === undefined and
  // Object.hasOwn(material, 'uniformsNeedUpdate') === false) on the
  // 3 shared color-only MeshBasic materials (wood / brass / steel;
  // measured uniformsNeedUpdate-absent 3).
  // pinColorOnlyUnlitBasicMaterialUniformsNeedUpdate /
  // pinColorOnlyVisualMaterialUniformsNeedUpdate run after
  // pinColorOnlyVisualMaterialUniforms.
  // Prefer delete material.uniformsNeedUpdate when the own property
  // is present. Do not assign null, undefined, false, or true. An
  // already-absent uniformsNeedUpdate is left alone. Pin once per
  // shared material instance. Do not invent a custom shader path. Do
  // not convert MeshBasic to ShaderMaterial. Do not clear
  // uniformsNeedUpdate on real ShaderMaterial / RawShaderMaterial.
  // Do not touch uniforms (v1.15.0 uniforms-absent 3 stays). Do not
  // touch defaultAttributeValues (v1.14.0 defaultAttributeValues-absent
  // 3 stays). Do not touch index0AttributeName (v1.13.0
  // index0AttributeName-absent 3 stays). Do not touch depthPacking
  // (v1.12.0 depthPacking-absent 3 stays). Do not touch extensions
  // (v1.11.0 extensions-absent 3 stays). Do not touch indirect
  // (v1.10.0 indirect-null 13 stays). Do not re-run the unused-channel
  // strip (v1.9.0 unusedAttributes-absent 13 stays). Do not touch
  // onUpload / onUploadCallback (v1.8.0 onUpload-release 13 stays).
  // Do not touch color (v1.7.0 colorAttribute-absent 13 stays). Do not
  // touch matrixWorldNeedsUpdate (v1.6.0 stays). Do not change
  // matrixAutoUpdate or matrixWorldAutoUpdate. Do not touch Material
  // version / name / userData or Mesh name / userData or
  // BufferGeometry name / userData. Do not touch BufferAttribute
  // fields. Do not touch prior Material program-cache / flag pins.
  // Do not touch uniformsGroups, vertexShader, fragmentShader, lights,
  // clipping, or isShaderMaterial. Do not touch bounds, morphs,
  // animations, shadows, frustumCulled, or mesh.visible.
  // Checked installed three@0.170.0:
  // Fresh Material / MeshBasicMaterial constructors do not assign
  // uniformsNeedUpdate. ShaderMaterial assigns
  // this.uniformsNeedUpdate = false. ShaderMaterial.copy does not
  // copy uniformsNeedUpdate; the copy keeps the constructor false.
  // RawShaderMaterial extends ShaderMaterial, so it keeps that
  // constructor flag. WebGLPrograms does not read uniformsNeedUpdate.
  // getUniforms still resolves shaderID from shaderIDs[material.type].
  // MeshBasic stays shaderID 'basic' and clones ShaderLib uniforms, so
  // a leftover uniformsNeedUpdate does not invent a custom shader
  // path. WebGLRenderer.setProgram re-uploads only when
  // material.isShaderMaterial && material.uniformsNeedUpdate === true,
  // then assigns material.uniformsNeedUpdate = false. MeshBasic is not
  // isShaderMaterial, so a leftover true, false, or null does not take
  // that extra upload. WebGLMaterials.refreshMaterialUniforms assigns
  // material.uniformsNeedUpdate = false only in the isShaderMaterial
  // branch (#15581). Assigning null, undefined, false, or true stores
  // an own property, which is not the r170 MeshBasic absence. Deleting
  // the own property restores the r170 absence. On clean procedural
  // materials draws / tris / attrBytes stay unchanged vs v1.15.0. No
  // invented headset ms.
  // Draws / tris / attrBytes unchanged vs v1.15.0 on clean procedural
  // meshes. uniforms-absent 3 stays. defaultAttributeValues-absent 3
  // stays. index0AttributeName-absent 3 stays. depthPacking-absent 3
  // stays. extensions-absent 3 stays. indirect-null 13 stays.
  // unusedAttributes-absent 13 stays. onUpload-release 13 stays.
  // colorAttribute-absent 13 stays. matrixWorldNeedsUpdate-false 13
  // stays. material-version-zero 3 stays. mesh-userData-empty 13
  // stays. mesh-name-empty 10 stays. unique MeshBasic 3 stays.
  // uniformsNeedUpdate-absent 3 is the new count.
  // v1.17.0: after that uniformsNeedUpdate pin, delete leftover
  // Material uniformsGroups so the r170 MeshBasic absence remains
  // (material.uniformsGroups === undefined and
  // Object.hasOwn(material, 'uniformsGroups') === false) on the 3
  // shared color-only MeshBasic materials (wood / brass / steel;
  // measured uniformsGroups-absent 3).
  // pinColorOnlyUnlitBasicMaterialUniformsGroups /
  // pinColorOnlyVisualMaterialUniformsGroups run after
  // pinColorOnlyVisualMaterialUniformsNeedUpdate.
  // Prefer delete material.uniformsGroups when the own property is
  // present. Do not assign null, undefined, [], or {}. An
  // already-absent uniformsGroups is left alone. Pin once per shared
  // material instance. Do not invent a custom shader path. Do not
  // convert MeshBasic to ShaderMaterial. Do not clear uniformsGroups
  // on real ShaderMaterial / RawShaderMaterial. Do not touch
  // uniformsNeedUpdate (v1.16.0 uniformsNeedUpdate-absent 3 stays).
  // Do not touch uniforms (v1.15.0 uniforms-absent 3 stays). Do not
  // touch defaultAttributeValues (v1.14.0 defaultAttributeValues-absent
  // 3 stays). Do not touch index0AttributeName (v1.13.0
  // index0AttributeName-absent 3 stays). Do not touch depthPacking
  ...
  // BufferGeometry name / userData. Do not touch BufferAttribute
  // fields. Do not touch prior Material program-cache / flag pins.
  // Do not touch vertexShader, fragmentShader, lights, clipping, or
  // isShaderMaterial. Do not touch bounds, morphs, animations,
  // shadows, frustumCulled, or mesh.visible.
  // Checked installed three@0.170.0:
  // Fresh Material / MeshBasicMaterial constructors do not assign
  // uniformsGroups. ShaderMaterial assigns this.uniformsGroups = [].
  // ShaderMaterial.copy assigns
  // this.uniformsGroups = cloneUniformsGroups(source.uniformsGroups),
  // which builds a new array and calls clone() on each group.
  // RawShaderMaterial extends ShaderMaterial, so it keeps that
  ...
  // getUniforms still resolves shaderID from shaderIDs[material.type].
  // MeshBasic stays shaderID 'basic' and clones ShaderLib uniforms, so
  // a leftover uniformsGroups array does not invent a custom shader
  // path and does not change draws, tris, or attrBytes.
  // WebGLRenderer.setProgram walks material.uniformsGroups and calls
  // WebGLUniformsGroups.update / bind (UBO create/bind) only when
  // material.isShaderMaterial || material.isRawShaderMaterial.
  // MeshBasic is neither, so a leftover array is not uploaded as
  // uniform buffer objects. Assigning null, undefined, [], or {}
  // stores an own property, which is not the r170 MeshBasic absence.
  // Deleting the own property restores the r170 absence. On clean
  // procedural materials draws / tris / attrBytes stay unchanged vs
  // v1.16.0. No invented headset ms.
  // Draws / tris / attrBytes unchanged vs v1.16.0 on clean procedural
  // meshes. uniformsNeedUpdate-absent 3 stays. uniforms-absent 3
  // stays. defaultAttributeValues-absent 3 stays.
  // index0AttributeName-absent 3 stays. depthPacking-absent 3 stays.
  // extensions-absent 3 stays. indirect-null 13 stays.
  // unusedAttributes-absent 13 stays. onUpload-release 13 stays.
  // colorAttribute-absent 13 stays. matrixWorldNeedsUpdate-false 13
  // stays. material-version-zero 3 stays. mesh-userData-empty 13
  // stays. mesh-name-empty 10 stays. unique MeshBasic 3 stays.
  // uniformsGroups-absent 3 is the new count.
  // v1.18.0: after that uniformsGroups pin, delete leftover Material
  // vertexShader so the r170 MeshBasic absence remains
  // (material.vertexShader === undefined and
  // Object.hasOwn(material, 'vertexShader') === false) on the 3
  // shared color-only MeshBasic materials (wood / brass / steel;
  // measured vertexShader-absent 3).
  // pinColorOnlyUnlitBasicMaterialVertexShader /
  // pinColorOnlyVisualMaterialVertexShader run after
  // pinColorOnlyVisualMaterialUniformsGroups.
  // Prefer delete material.vertexShader when the own property is
  // present. Do not assign null, undefined, '', or a stub GLSL
  // string. An already-absent vertexShader is left alone. Pin once
  // per shared material instance. Do not invent a custom shader path.
  // Do not convert MeshBasic to ShaderMaterial. Do not clear
  // vertexShader on real ShaderMaterial / RawShaderMaterial. Do not
  // touch uniformsGroups (v1.17.0 uniformsGroups-absent 3 stays). Do
  // not touch uniformsNeedUpdate (v1.16.0 uniformsNeedUpdate-absent 3
  // stays). Do not touch uniforms (v1.15.0 uniforms-absent 3 stays).
  // Do not touch defaultAttributeValues (v1.14.0
  // defaultAttributeValues-absent 3 stays). Do not touch
  // index0AttributeName (v1.13.0 index0AttributeName-absent 3 stays).
  // Do not touch depthPacking (v1.12.0 depthPacking-absent 3 stays).
  // Do not touch extensions (v1.11.0 extensions-absent 3 stays). Do
  ...
  // BufferGeometry name / userData. Do not touch BufferAttribute
  // fields. Do not touch prior Material program-cache / flag pins.
  // Do not touch fragmentShader, lights, clipping, or isShaderMaterial.
  // Do not touch bounds, morphs, animations, shadows, frustumCulled,
  // or mesh.visible.
  // Checked installed three@0.170.0:
  // Fresh Material / MeshBasicMaterial constructors do not assign
  // vertexShader. Material.js does not mention vertexShader;
  // Material.copy and MeshBasicMaterial.copy do not copy it.
  // ShaderMaterial assigns this.vertexShader = default_vertex.
  // ShaderMaterial.copy assigns this.vertexShader = source.vertexShader
  // (the source string, not a rewritten stub). ShaderMaterial.toJSON
  // writes data.vertexShader = this.vertexShader. RawShaderMaterial
  // extends ShaderMaterial and does not assign its own vertexShader,
  // so it keeps that constructor string. MaterialLoader.parse assigns
  // material.vertexShader = json.vertexShader when json.vertexShader
  // !== undefined, including onto a MeshBasic.
  // WebGLPrograms.getParameters resolves shaderID from
  // shaderIDs[material.type]. When shaderID is set (MeshBasicMaterial
  // maps to 'basic'), vertexShader comes from
  // ShaderLib[shaderID].vertexShader, not material.vertexShader. The
  // custom path (vertexShader = material.vertexShader and
  // WebGLShaderCache.update) runs only when shaderID is absent.
  ...
  // r170 MeshBasic absence. Deleting the own property restores the r170
  // absence. On clean procedural materials draws / tris / attrBytes
  // stay unchanged vs v1.17.0. No invented headset ms.
  // Draws / tris / attrBytes unchanged vs v1.17.0 on clean procedural
  // meshes. uniformsGroups-absent 3 stays. uniformsNeedUpdate-absent 3
  // stays. uniforms-absent 3 stays. defaultAttributeValues-absent 3
  // stays. index0AttributeName-absent 3 stays. depthPacking-absent 3
  // stays. extensions-absent 3 stays. indirect-null 13 stays.
  // unusedAttributes-absent 13 stays. onUpload-release 13 stays.
  // colorAttribute-absent 13 stays. matrixWorldNeedsUpdate-false 13
  // stays. material-version-zero 3 stays. mesh-userData-empty 13
  // stays. mesh-name-empty 10 stays. unique MeshBasic 3 stays.
  // vertexShader-absent 3 is the new count.
