...
test("packaged ingest pins leftover indirect null on color-only visuals and the fastener", () => {
  const { root, groups, fastener, lid, latch } = makePackagedFixture();
  const bodyMesh = groups[0][0].children.find((o) => o.isMesh);
  const bodyIndirect = { label: "body-indirect" };
  bodyMesh.geometry.setIndirect(bodyIndirect);
  const bodyBefore = cpuAttrBytes(bodyMesh.geometry);
  const bodyChannels = channelByteLength(bodyMesh.geometry);
  const bodyPositionBytes = bodyMesh.geometry.getAttribute("position").array.byteLength;
  const fastenerIndirect = { label: "fastener-indirect" };
  fastener.geometry.setIndirect(fastenerIndirect);
  fastener.material = new THREE.MeshBasicMaterial({ color: 0xbe7e31 });
  const fastenerMat = fastener.material;
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  ...
  interleaved.name = "interleavedMesh";
  const lit = boxMesh("litMesh", new THREE.MeshStandardMaterial());
  const litIndirect = { label: "lit-indirect" };
  lit.geometry.setIndirect(litIndirect);
  groups[0][0].add(mapped, interleaved, lit);
  root.add(sharedGeoVisual);
  const colliderGrab = root.getObjectByName("collider_grab");
  const colliderIndirect = { label: "collider-indirect" };
  colliderGrab.geometry.setIndirect(colliderIndirect);
  const lidMesh = lid.getObjectByName("lidL0");
  const latchMesh = latch.getObjectByName("latchL0");
  const lidIndirect = { label: "lid-indirect" };
  const latchIndirect = { label: "latch-indirect" };
  lidMesh.geometry.setIndirect(lidIndirect);
  latchMesh.geometry.setIndirect(latchIndirect);
  lidMesh.name = "lidMesh";
  latchMesh.name = "latchMesh";

  ingestPackagedRoot(root, sidecar);

  assert.equal(bodyMesh.geometry.indirect, null, "in-place color-only body indirect is cleared");
  assert.equal(bodyMesh.geometry.getIndirect(), null, "body getIndirect is null");
  assert.equal(
    cpuAttrBytes(bodyMesh.geometry),
    bodyBefore - bodyChannels - bodyPositionBytes / 2,
    "byte drop is unused channels plus Float16 position quantize; clearing indirect adds no attribute bytes",
  );
  assert.equal(bodyChannels, 480, "BoxGeometry leftover uv + normal is still 480 bytes");
  assert.equal(bodyPositionBytes, 288, "24-vert Float32 position is 288 bytes before quantize");
  assert.equal(bodyMesh.geometry.getAttribute("position").count, 24, "packed body position count stays");
  assert.ok(bodyMesh.geometry.index, "packed body index stays");
  assert.equal(unusedChannelAttributesAbsent(bodyMesh.geometry), true, "unused-channel strip still runs before the indirect pin");
  assert.equal(geometryOnUploadRelease(bodyMesh.geometry), true, "onUpload release hook still runs");
  assert.equal(fastener.geometry.indirect, null, "fastener indirect is cleared");
  assert.equal(fastener.geometry.getAttribute("position").count, 24, "fastener position count stays");
  assert.ok(fastener.geometry.index, "fastener index stays");
  assert.equal(fastener.material, fastenerMat, "ingest does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fastenerMesh stays named");
  assert.equal(lidMesh.geometry.indirect, null, "reserved lidMesh indirect is cleared");
  assert.equal(latchMesh.geometry.indirect, null, "reserved latchMesh indirect is cleared");
  assert.equal(lidMesh.name, "lidMesh", "reserved lidMesh name stays");
  assert.equal(latchMesh.name, "latchMesh", "reserved latchMesh name stays");
  assert.equal(mapped.geometry.getIndirect(), mappedIndirect, "mapped indirect stays authored");
  assert.equal(mapped.geometry.getAttribute("uv"), mappedUv, "mapped uv stays authored");
  assert.equal(sharedGeoVisual.geometry.getIndirect(), mappedIndirect, "geometry shared with a mapped mesh keeps indirect");
  assert.equal(sharedGeoVisual.name, "sharedGeoBody", "shared-geometry mesh name stays");
  assert.equal(interleaved.geometry.getIndirect(), interleavedIndirect, "interleaved indirect stays authored");
  assert.equal(lit.geometry.getIndirect(), litIndirect, "MeshStandard indirect stays authored");
  assert.equal(colliderGrab.geometry.getIndirect(), colliderIndirect, "collider indirect stays authored");
  assert.equal(colliderGrab.name, "collider_grab", "collider mesh name stays");
});

test("packaged ingest without lod groups still pins leftover indirect null", () => {
  const { root, body, lid, latch, fastener } = makePackagedFixture({ withLod: false });
  const bodyMesh = visualMeshes(body)[0];
  const bodyIndirect = { label: "fail-soft-body-indirect" };
  bodyMesh.geometry.setIndirect(bodyIndirect);
  const bodyChannels = channelByteLength(bodyMesh.geometry);
  const bodyBefore = cpuAttrBytes(bodyMesh.geometry);
  const bodyPosition = bodyMesh.geometry.getAttribute("position");
  ...
  const bodyBag = bodyMesh.userData;
  const alphaMesh = boxMesh("alphaBody", new THREE.MeshBasicMaterial({ color: 0x633318 }));
  const alphaIndirect = { label: "fail-soft-alpha-indirect" };
  alphaMesh.geometry.setIndirect(alphaIndirect);
  const alphaBefore = cpuAttrBytes(alphaMesh.geometry);
  const alphaChannels = channelByteLength(alphaMesh.geometry);
  body.add(alphaMesh);
  const fastenerIndirect = { label: "fail-soft-fastener-indirect" };
  fastener.geometry.setIndirect(fastenerIndirect);
  const fastenerMat = fastener.material;
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  const mappedIndirect = { label: "fail-soft-mapped-indirect" };
  mapped.geometry.setIndirect(mappedIndirect);
  const mappedUv = mapped.geometry.getAttribute("uv");
  body.add(mapped);
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  ...
  body.add(sharedGeoVisual);
  const colliderGrab = root.getObjectByName("collider_grab");
  const colliderIndirect = { label: "fail-soft-collider-indirect" };
  colliderGrab.geometry.setIndirect(colliderIndirect);
  const lidMesh = lid.getObjectByName("lidMesh");
  const latchMesh = latch.getObjectByName("latchMesh");
  const lidIndirect = { label: "fail-soft-lid-indirect" };
  const latchIndirect = { label: "fail-soft-latch-indirect" };
  lidMesh.geometry.setIndirect(lidIndirect);
  latchMesh.geometry.setIndirect(latchIndirect);
  ...
  assert.equal(root.userData.lod, undefined, "fail-soft still does not invent lod groups");
  assert.equal(bodyMesh.material, bodyMat, "fail-soft does not replace the body material");
  assert.equal(bodyMesh.geometry.indirect, null, "fail-soft body indirect is cleared");
  assert.equal(bodyMesh.geometry.getIndirect(), null, "fail-soft body getIndirect is null");
  assert.equal(cpuAttrBytes(bodyMesh.geometry), bodyBefore - bodyChannels, "fail-soft indirect clear does not change the channel byte drop");
  assert.equal(bodyMesh.geometry.getAttribute("position"), bodyPosition, "fail-soft pin does not replace position");
  assert.equal(bodyMesh.geometry.index, bodyIndex, "fail-soft pin does not replace the index");
  assert.equal(bodyMesh.name, "lidMesh", "fail-soft reserved lidMesh name stays");
  assert.equal(bodyMesh.userData, bodyBag, "fail-soft mesh.userData identity stays when already empty");
  ...
  assert.equal(bodyMesh.material.version, 0, "fail-soft material.version pin still runs");
  assert.equal(geometryOnUploadRelease(bodyMesh.geometry), true, "fail-soft onUpload release hook still runs");
  assert.equal(unusedChannelAttributesAbsent(bodyMesh.geometry), true, "fail-soft unused channels are still stripped");
  assert.equal(alphaMesh.geometry.indirect, null, "fail-soft second color-only mesh indirect is cleared");
  assert.equal(cpuAttrBytes(alphaMesh.geometry), alphaBefore - alphaChannels, "fail-soft second mesh byte drop is still uv and normal");
  assert.equal(fastener.geometry.indirect, null, "fail-soft fastener indirect is cleared");
  assert.ok(fastener.geometry.getAttribute("position"), "fail-soft fastener position stays");
  assert.ok(fastener.geometry.index, "fail-soft fastener index stays");
  assert.equal(fastener.material, fastenerMat, "fail-soft does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fail-soft fastenerMesh stays named");
  assert.equal(lidMesh.geometry.indirect, null, "fail-soft lidMesh indirect is cleared");
  assert.equal(lidMesh.name, "lidMesh", "fail-soft authored lidMesh name stays");
  assert.equal(latchMesh.geometry.indirect, null, "fail-soft latchMesh indirect is cleared");
  assert.equal(latchMesh.name, "latchMesh", "fail-soft latchMesh name stays");
  assert.equal(mapped.geometry.getIndirect(), mappedIndirect, "fail-soft mapped indirect stays authored");
  assert.equal(mapped.geometry.getAttribute("uv"), mappedUv, "fail-soft mapped uv stays authored");
  assert.equal(sharedGeoVisual.geometry.getIndirect(), mappedIndirect, "fail-soft geometry shared with a mapped mesh keeps indirect");
  assert.equal(sharedGeoVisual.name, "sharedGeoBody", "fail-soft shared-geometry mesh name stays");
  assert.equal(sharedGeoMat.version, 6, "fail-soft shared material.version stays");
  assert.equal(colliderGrab.geometry.getIndirect(), colliderIndirect, "fail-soft collider indirect stays");
  assert.equal(colliderGrab.name, "collider_grab", "fail-soft collider name stays");
});
...
test("packaged ingest deletes leftover Material extensions on color-only visuals and the fastener", () => {
  const { root, groups, fastener, lid, latch } = makePackagedFixture();
  const bodyMesh = groups[0][0].children.find((o) => o.isMesh);
  const bodyExtensions = { multiDraw: true };
  bodyMesh.material.extensions = bodyExtensions;
  const bodyIndirect = { label: "body-indirect" };
  bodyMesh.geometry.setIndirect(bodyIndirect);
  const bodyMat = bodyMesh.material;
  bodyMat.version = 4;
  const fastenerExtensions = { clipCullDistance: true };
  fastener.material = new THREE.MeshBasicMaterial({ color: 0xbe7e31 });
  fastener.material.extensions = fastenerExtensions;
  const fastenerMat = fastener.material;
  const fastenerIndirect = { label: "fastener-indirect" };
  fastener.geometry.setIndirect(fastenerIndirect);
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  const mappedExtensions = { multiDraw: true };
  mapped.material.extensions = mappedExtensions;
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  const sharedGeoExtensions = {};
  ...
  interleavedGeo.setAttribute("position", new THREE.InterleavedBufferAttribute(interleavedBuffer, 3, 0));
  const interleavedMat = new THREE.MeshBasicMaterial({ color: 0x633318 });
  const interleavedExtensions = { clipCullDistance: false, multiDraw: false };
  interleavedMat.extensions = interleavedExtensions;
  const interleaved = new THREE.Mesh(interleavedGeo, interleavedMat);
  interleaved.name = "interleavedMesh";
  const shader = new THREE.ShaderMaterial();
  const shaderExtensions = shader.extensions;
  const shaderMesh = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.1, 0.1), shader);
  shaderMesh.name = "shaderLike";
  const lit = boxMesh("litMesh", new THREE.MeshStandardMaterial());
  const litExtensions = { multiDraw: true };
  lit.material.extensions = litExtensions;
  groups[0][0].add(mapped, interleaved, shaderMesh, lit);
  root.add(sharedGeoVisual);
  const colliderGrab = root.getObjectByName("collider_grab");
  const colliderExtensions = { clipCullDistance: true };
  colliderGrab.material.extensions = colliderExtensions;
  const lidMesh = lid.getObjectByName("lidL0");
  const latchMesh = latch.getObjectByName("latchL0");
  const lidExtensions = { multiDraw: true };
  const latchExtensions = {};
  lidMesh.material.extensions = lidExtensions;
  latchMesh.material.extensions = latchExtensions;
  lidMesh.name = "lidMesh";
  latchMesh.name = "latchMesh";
  ...
  assert.equal(materialExtensionsAbsent(bodyMesh.material), true, "in-place color-only body extensions is deleted");
  assert.equal(bodyExtensions.multiDraw, true, "pin does not mutate the detached extensions object");
  assert.equal(bodyMesh.geometry.indirect, null, "indirect pin still runs before the extensions pin");
  assert.equal(bodyMesh.material.version, 0, "material.version pin still runs");
  assert.equal(unusedChannelAttributesAbsent(bodyMesh.geometry), true, "unused-channel strip still runs");
  assert.equal(materialExtensionsAbsent(fastener.material), true, "fastener extensions is deleted");
  assert.equal(fastener.geometry.indirect, null, "fastener indirect is still cleared");
  assert.equal(fastener.material, fastenerMat, "ingest does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fastenerMesh stays named");
  assert.equal(materialExtensionsAbsent(lidMesh.material), true, "reserved lidMesh extensions is deleted");
  assert.equal(materialExtensionsAbsent(latchMesh.material), true, "reserved latchMesh extensions is deleted");
  assert.equal(lidMesh.name, "lidMesh", "reserved lidMesh name stays");
  assert.equal(latchMesh.name, "latchMesh", "reserved latchMesh name stays");
  assert.equal(mapped.material.extensions, mappedExtensions, "mapped extensions stays authored");
  assert.equal(sharedGeoVisual.material.extensions, sharedGeoExtensions, "geometry shared with a mapped mesh keeps extensions");
  assert.equal(sharedGeoVisual.name, "sharedGeoBody", "shared-geometry mesh name stays");
  assert.equal(interleaved.material.extensions, interleavedExtensions, "interleaved extensions stays authored");
  assert.equal(shader.extensions, shaderExtensions, "ShaderMaterial extensions stays authored");
  assert.equal(lit.material.extensions, litExtensions, "MeshStandard extensions stays authored");
  assert.equal(colliderGrab.material.extensions, colliderExtensions, "collider extensions stays authored");
  assert.equal(colliderGrab.name, "collider_grab", "collider mesh name stays");
});

test("packaged ingest without lod groups still deletes leftover Material extensions", () => {
  const { root, body, lid, latch, fastener } = makePackagedFixture({ withLod: false });
  const bodyMesh = visualMeshes(body)[0];
  const bodyExtensions = { multiDraw: true, clipCullDistance: true };
  bodyMesh.material.extensions = bodyExtensions;
  const bodyIndirect = { label: "fail-soft-body-indirect" };
  bodyMesh.geometry.setIndirect(bodyIndirect);
  const bodyMat = bodyMesh.material;
  ...
  const bodyBag = bodyMesh.userData;
  const alphaMesh = boxMesh("alphaBody", new THREE.MeshBasicMaterial({ color: 0x633318 }));
  const sharedAlpha = alphaMesh.material;
  alphaMesh.material.extensions = {};
  const alphaTwin = boxMesh("alphaTwin", sharedAlpha);
  body.add(alphaMesh, alphaTwin);
  const fastenerExtensions = { clipCullDistance: true };
  fastener.material.extensions = fastenerExtensions;
  const fastenerMat = fastener.material;
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  ...
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  sharedGeoMat.version = 6;
  const sharedGeoExtensions = { multiDraw: false };
  sharedGeoMat.extensions = sharedGeoExtensions;
  const sharedGeoVisual = new THREE.Mesh(mapped.geometry, sharedGeoMat);
  sharedGeoVisual.name = "sharedGeoBody";
  body.add(sharedGeoVisual);
  const shader = new THREE.ShaderMaterial();
  const shaderExtensions = shader.extensions;
  const shaderMesh = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.05, 0.05), shader);
  shaderMesh.name = "shaderLike";
  body.add(shaderMesh);
  const colliderGrab = root.getObjectByName("collider_grab");
  ...
  const lidMesh = lid.getObjectByName("lidMesh");
  const latchMesh = latch.getObjectByName("latchMesh");
  lidMesh.material.extensions = { clipCullDistance: true };
  latchMesh.material.extensions = { multiDraw: true };

  ingestPackagedRoot(root, sidecar);

  assert.equal(root.userData.lod, undefined, "fail-soft still does not invent lod groups");
  assert.equal(bodyMesh.material, bodyMat, "fail-soft does not replace the body material");
  assert.equal(materialExtensionsAbsent(bodyMesh.material), true, "fail-soft body extensions is deleted");
  assert.equal(bodyMesh.geometry.indirect, null, "fail-soft indirect pin still runs");
  assert.equal(bodyMesh.name, "lidMesh", "fail-soft reserved lidMesh name stays");
  assert.equal(bodyMesh.userData, bodyBag, "fail-soft mesh.userData identity stays when already empty");
  assert.equal(bodyMesh.matrixWorldNeedsUpdate, false, "fail-soft matrixWorldNeedsUpdate stays false");
  assert.equal(bodyMesh.material.version, 0, "fail-soft material.version pin still runs");
  assert.equal(materialExtensionsAbsent(alphaMesh.material), true, "fail-soft shared color-only extensions is deleted once");
  assert.equal(alphaTwin.material, sharedAlpha, "fail-soft second mesh keeps the shared material");
  assert.equal(materialExtensionsAbsent(fastener.material), true, "fail-soft fastener extensions is deleted");
  assert.equal(fastener.material, fastenerMat, "fail-soft does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fail-soft fastenerMesh stays named");
  assert.equal(materialExtensionsAbsent(lidMesh.material), true, "fail-soft lidMesh extensions is deleted");
  assert.equal(lidMesh.name, "lidMesh", "fail-soft authored lidMesh name stays");
  assert.equal(materialExtensionsAbsent(latchMesh.material), true, "fail-soft latchMesh extensions is deleted");
  assert.equal(latchMesh.name, "latchMesh", "fail-soft latchMesh name stays");
  assert.equal(mapped.material.extensions, mappedExtensions, "fail-soft mapped extensions stays authored");
  assert.equal(sharedGeoVisual.material.extensions, sharedGeoExtensions, "fail-soft geometry shared with a mapped mesh keeps extensions");
  assert.equal(sharedGeoMat.version, 6, "fail-soft shared material.version stays");
  assert.equal(shader.extensions, shaderExtensions, "fail-soft ShaderMaterial extensions stays authored");
  assert.equal(colliderGrab.material.extensions, colliderExtensions, "fail-soft collider extensions stays");
  assert.equal(colliderGrab.name, "collider_grab", "fail-soft collider name stays");
});
...
test("packaged ingest deletes leftover Material depthPacking on color-only visuals and the fastener", () => {
  const { root, groups, fastener, lid, latch } = makePackagedFixture();
  const bodyMesh = groups[0][0].children.find((o) => o.isMesh);
  const bodyExtensions = { multiDraw: true };
  bodyMesh.material.extensions = bodyExtensions;
  bodyMesh.material.depthPacking = THREE.BasicDepthPacking;
  bodyMesh.material.index0AttributeName = "position";
  const bodyIndirect = { label: "body-indirect" };
  bodyMesh.geometry.setIndirect(bodyIndirect);
  const bodyMat = bodyMesh.material;
  bodyMat.version = 4;
  fastener.material = new THREE.MeshBasicMaterial({ color: 0xbe7e31 });
  fastener.material.depthPacking = THREE.RGBADepthPacking;
  const fastenerMat = fastener.material;
  const fastenerIndirect = { label: "fastener-indirect" };
  fastener.geometry.setIndirect(fastenerIndirect);
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  mapped.material.depthPacking = THREE.BasicDepthPacking;
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  ...
  const shader = new THREE.ShaderMaterial();
  shader.depthPacking = THREE.BasicDepthPacking;
  const shaderMesh = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.1, 0.1), shader);
  shaderMesh.name = "shaderLike";
  const lit = boxMesh("litMesh", new THREE.MeshStandardMaterial());
  lit.material.depthPacking = THREE.RGBADepthPacking;
  groups[0][0].add(mapped, interleaved, depthMesh, shaderMesh, lit);
  root.add(sharedGeoVisual);
  const colliderGrab = root.getObjectByName("collider_grab");
  colliderGrab.material.depthPacking = 0;
  const lidMesh = lid.getObjectByName("lidL0");
  const latchMesh = latch.getObjectByName("latchL0");
  lidMesh.material.depthPacking = THREE.BasicDepthPacking;
  latchMesh.material.depthPacking = 0;
  lidMesh.name = "lidMesh";
  latchMesh.name = "latchMesh";
  ...
  assert.equal(bodyMesh.material, bodyMat, "ingest does not replace the body material");
  assert.equal(bodyMesh.material.isMeshBasicMaterial, true, "body stays MeshBasic");
  assert.equal(bodyMesh.material.isMeshDepthMaterial, undefined, "body is not converted to MeshDepthMaterial");
  assert.equal(materialDepthPackingAbsent(bodyMesh.material), true, "in-place color-only body depthPacking is deleted");
  assert.equal(materialExtensionsAbsent(bodyMesh.material), true, "extensions pin still clears leftover extensions");
  assert.equal(bodyExtensions.multiDraw, true, "pin does not mutate the detached extensions object");
  assert.equal(bodyMesh.material.index0AttributeName, undefined, "ingest deletes leftover index0AttributeName");
  assert.equal(Object.hasOwn(bodyMesh.material, "index0AttributeName"), false, "ingest removes the index0AttributeName own property");
  assert.equal(bodyMesh.geometry.indirect, null, "indirect pin still runs before the depthPacking pin");
  assert.equal(bodyMesh.material.version, 0, "material.version pin still runs");
  assert.equal(unusedChannelAttributesAbsent(bodyMesh.geometry), true, "unused-channel strip still runs");
  assert.equal(materialDepthPackingAbsent(fastener.material), true, "fastener depthPacking is deleted");
  assert.equal(fastener.geometry.indirect, null, "fastener indirect is still cleared");
  assert.equal(fastener.material, fastenerMat, "ingest does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fastenerMesh stays named");
  assert.equal(materialDepthPackingAbsent(lidMesh.material), true, "reserved lidMesh depthPacking is deleted");
  assert.equal(materialDepthPackingAbsent(latchMesh.material), true, "reserved latchMesh depthPacking is deleted");
  assert.equal(lidMesh.name, "lidMesh", "reserved lidMesh name stays");
  assert.equal(latchMesh.name, "latchMesh", "reserved latchMesh name stays");
  assert.equal(mapped.material.depthPacking, THREE.BasicDepthPacking, "mapped depthPacking stays authored");
  ...
  assert.equal(sharedGeoVisual.name, "sharedGeoBody", "shared-geometry mesh name stays");
  assert.equal(interleaved.material.depthPacking, THREE.RGBADepthPacking, "interleaved depthPacking stays authored");
  assert.equal(depthMat.depthPacking, THREE.BasicDepthPacking, "MeshDepthMaterial depthPacking stays authored");
  assert.equal(shader.depthPacking, THREE.BasicDepthPacking, "ShaderMaterial depthPacking stays authored");
  assert.equal(lit.material.depthPacking, THREE.RGBADepthPacking, "MeshStandard depthPacking stays authored");
  assert.equal(colliderGrab.material.depthPacking, 0, "collider depthPacking stays authored");
  assert.equal(Object.hasOwn(colliderGrab.material, "depthPacking"), true, "collider own depthPacking stays");
  assert.equal(colliderGrab.name, "collider_grab", "collider mesh name stays");
});

test("packaged ingest without lod groups still deletes leftover Material depthPacking", () => {
  const { root, body, lid, latch, fastener } = makePackagedFixture({ withLod: false });
  const bodyMesh = visualMeshes(body)[0];
  bodyMesh.material.depthPacking = THREE.BasicDepthPacking;
  bodyMesh.material.extensions = { multiDraw: true, clipCullDistance: true };
  const bodyIndirect = { label: "fail-soft-body-indirect" };
  bodyMesh.geometry.setIndirect(bodyIndirect);
  const bodyMat = bodyMesh.material;
  bodyMesh.name = "lidMesh";
  ...
  const alphaMesh = boxMesh("alphaBody", new THREE.MeshBasicMaterial({ color: 0x633318 }));
  const sharedAlpha = alphaMesh.material;
  alphaMesh.material.depthPacking = THREE.RGBADepthPacking;
  const alphaTwin = boxMesh("alphaTwin", sharedAlpha);
  body.add(alphaMesh, alphaTwin);
  fastener.material.depthPacking = 0;
  const fastenerMat = fastener.material;
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  mapped.material.depthPacking = THREE.BasicDepthPacking;
  body.add(mapped);
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  sharedGeoMat.version = 6;
  sharedGeoMat.depthPacking = THREE.RGBADepthPacking;
  const sharedGeoVisual = new THREE.Mesh(mapped.geometry, sharedGeoMat);
  sharedGeoVisual.name = "sharedGeoBody";
  body.add(sharedGeoVisual);
  const depthMat = new THREE.MeshDepthMaterial();
  const depthMesh = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.05, 0.05), depthMat);
  depthMesh.name = "depthLike";
  body.add(depthMesh);
  const shader = new THREE.ShaderMaterial();
  ...
  body.add(shaderMesh);
  const colliderGrab = root.getObjectByName("collider_grab");
  colliderGrab.material.depthPacking = THREE.BasicDepthPacking;
  const lidMesh = lid.getObjectByName("lidMesh");
  const latchMesh = latch.getObjectByName("latchMesh");
  lidMesh.material.depthPacking = THREE.RGBADepthPacking;
  latchMesh.material.depthPacking = THREE.BasicDepthPacking;

  ingestPackagedRoot(root, sidecar);

  assert.equal(root.userData.lod, undefined, "fail-soft still does not invent lod groups");
  assert.equal(bodyMesh.material, bodyMat, "fail-soft does not replace the body material");
  assert.equal(materialDepthPackingAbsent(bodyMesh.material), true, "fail-soft body depthPacking is deleted");
  assert.equal(materialExtensionsAbsent(bodyMesh.material), true, "fail-soft extensions pin still runs");
  assert.equal(bodyMesh.material.index0AttributeName, undefined, "fail-soft deletes leftover index0AttributeName");
  assert.equal(Object.hasOwn(bodyMesh.material, "index0AttributeName"), false, "fail-soft removes the index0AttributeName own property");
  assert.equal(bodyMesh.geometry.indirect, null, "fail-soft indirect pin still runs");
  assert.equal(bodyMesh.name, "lidMesh", "fail-soft reserved lidMesh name stays");
  assert.equal(bodyMesh.userData, bodyBag, "fail-soft mesh.userData identity stays when already empty");
  assert.equal(bodyMesh.matrixWorldNeedsUpdate, false, "fail-soft matrixWorldNeedsUpdate stays false");
  assert.equal(bodyMesh.material.version, 0, "fail-soft material.version pin still runs");
  assert.equal(materialDepthPackingAbsent(alphaMesh.material), true, "fail-soft shared color-only depthPacking is deleted once");
  assert.equal(alphaTwin.material, sharedAlpha, "fail-soft second mesh keeps the shared material");
  assert.equal(materialDepthPackingAbsent(fastener.material), true, "fail-soft fastener depthPacking is deleted");
  assert.equal(fastener.material, fastenerMat, "fail-soft does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fail-soft fastenerMesh stays named");
  assert.equal(materialDepthPackingAbsent(lidMesh.material), true, "fail-soft lidMesh depthPacking is deleted");
  assert.equal(lidMesh.name, "lidMesh", "fail-soft authored lidMesh name stays");
  assert.equal(materialDepthPackingAbsent(latchMesh.material), true, "fail-soft latchMesh depthPacking is deleted");
  assert.equal(latchMesh.name, "latchMesh", "fail-soft latchMesh name stays");
  assert.equal(mapped.material.depthPacking, THREE.BasicDepthPacking, "fail-soft mapped depthPacking stays authored");
  assert.equal(sharedGeoVisual.material.depthPacking, THREE.RGBADepthPacking, "fail-soft geometry shared with a mapped mesh keeps depthPacking");
  assert.equal(sharedGeoMat.version, 6, "fail-soft shared material.version stays");
  assert.equal(depthMat.depthPacking, THREE.BasicDepthPacking, "fail-soft MeshDepthMaterial depthPacking stays authored");
  assert.equal(shader.depthPacking, 0, "fail-soft ShaderMaterial depthPacking stays authored");
  assert.equal(colliderGrab.material.depthPacking, THREE.BasicDepthPacking, "fail-soft collider depthPacking stays");
  assert.equal(colliderGrab.name, "collider_grab", "fail-soft collider name stays");
});

function materialIndex0AttributeNameAbsent(material) {
  return material?.index0AttributeName === undefined && Object.hasOwn(material, "index0AttributeName") === false;
}

test("packaged ingest deletes leftover Material index0AttributeName on color-only visuals and the fastener", () => {
  const { root, groups, fastener, lid, latch } = makePackagedFixture();
  const bodyMesh = groups[0][0].children.find((o) => o.isMesh);
  const bodyExtensions = { multiDraw: true };
  bodyMesh.material.extensions = bodyExtensions;
  ...
  bodyMat.version = 4;
  fastener.material = new THREE.MeshBasicMaterial({ color: 0xbe7e31 });
  fastener.material.index0AttributeName = "color";
  const fastenerMat = fastener.material;
  const fastenerIndirect = { label: "fastener-indirect" };
  fastener.geometry.setIndirect(fastenerIndirect);
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  mapped.material.index0AttributeName = "uv";
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  sharedGeoMat.index0AttributeName = "";
  const sharedGeoVisual = new THREE.Mesh(mapped.geometry, sharedGeoMat);
  sharedGeoVisual.name = "sharedGeoBody";
  const interleavedGeo = new THREE.BufferGeometry();
  const interleavedBuffer = new THREE.InterleavedBuffer(new Float32Array([0, 0, 0, 1, 0, 0]), 3);
  interleavedGeo.setAttribute("position", new THREE.InterleavedBufferAttribute(interleavedBuffer, 3, 0));
  const interleavedMat = new THREE.MeshBasicMaterial({ color: 0x633318 });
  interleavedMat.index0AttributeName = "position";
  const interleaved = new THREE.Mesh(interleavedGeo, interleavedMat);
  interleaved.name = "interleavedMesh";
  const shader = new THREE.ShaderMaterial();
  shader.index0AttributeName = "position";
  const shaderMesh = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.1, 0.1), shader);
  shaderMesh.name = "shaderLike";
  const lit = boxMesh("litMesh", new THREE.MeshStandardMaterial());
  lit.material.index0AttributeName = "color";
  groups[0][0].add(mapped, interleaved, shaderMesh, lit);
  root.add(sharedGeoVisual);
  const colliderGrab = root.getObjectByName("collider_grab");
  colliderGrab.material.index0AttributeName = "";
  const lidMesh = lid.getObjectByName("lidL0");
  const latchMesh = latch.getObjectByName("latchL0");
  lidMesh.material.index0AttributeName = "position";
  latchMesh.material.index0AttributeName = "";
  lidMesh.name = "lidMesh";
  latchMesh.name = "latchMesh";
  ...
  assert.equal(bodyMesh.material, bodyMat, "ingest does not replace the body material");
  assert.equal(bodyMesh.material.isMeshBasicMaterial, true, "body stays MeshBasic");
  assert.equal(materialIndex0AttributeNameAbsent(bodyMesh.material), true, "in-place color-only body index0AttributeName is deleted");
  assert.equal(materialDepthPackingAbsent(bodyMesh.material), true, "depthPacking pin still clears leftover depthPacking");
  assert.equal(materialExtensionsAbsent(bodyMesh.material), true, "extensions pin still clears leftover extensions");
  assert.equal(bodyExtensions.multiDraw, true, "pin does not mutate the detached extensions object");
  assert.equal(bodyMesh.geometry.indirect, null, "indirect pin still runs before the index0AttributeName pin");
  assert.equal(bodyMesh.material.version, 0, "material.version pin still runs");
  assert.equal(unusedChannelAttributesAbsent(bodyMesh.geometry), true, "unused-channel strip still runs");
  assert.equal(materialIndex0AttributeNameAbsent(fastener.material), true, "fastener index0AttributeName is deleted");
  assert.equal(fastener.geometry.indirect, null, "fastener indirect is still cleared");
  assert.equal(fastener.material, fastenerMat, "ingest does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fastenerMesh stays named");
  assert.equal(materialIndex0AttributeNameAbsent(lidMesh.material), true, "reserved lidMesh index0AttributeName is deleted");
  assert.equal(materialIndex0AttributeNameAbsent(latchMesh.material), true, "reserved latchMesh index0AttributeName is deleted");
  assert.equal(lidMesh.name, "lidMesh", "reserved lidMesh name stays");
  assert.equal(latchMesh.name, "latchMesh", "reserved latchMesh name stays");
  assert.equal(mapped.material.index0AttributeName, "uv", "mapped index0AttributeName stays authored");
  assert.equal(sharedGeoVisual.material.index0AttributeName, "", "geometry shared with a mapped mesh keeps index0AttributeName");
  assert.equal(Object.hasOwn(sharedGeoMat, "index0AttributeName"), true, "shared-geometry empty string stays an own property");
  assert.equal(sharedGeoVisual.name, "sharedGeoBody", "shared-geometry mesh name stays");
  assert.equal(interleaved.material.index0AttributeName, "position", "interleaved index0AttributeName stays authored");
  assert.equal(shader.index0AttributeName, "position", "ShaderMaterial index0AttributeName stays authored");
  assert.equal(lit.material.index0AttributeName, "color", "MeshStandard index0AttributeName stays authored");
  assert.equal(colliderGrab.material.index0AttributeName, "", "collider index0AttributeName stays authored");
  assert.equal(Object.hasOwn(colliderGrab.material, "index0AttributeName"), true, "collider own index0AttributeName stays");
  assert.equal(colliderGrab.name, "collider_grab", "collider mesh name stays");
});

test("packaged ingest without lod groups still deletes leftover Material index0AttributeName", () => {
  const { root, body, lid, latch, fastener } = makePackagedFixture({ withLod: false });
  const bodyMesh = visualMeshes(body)[0];
  bodyMesh.material.index0AttributeName = "position";
  bodyMesh.material.depthPacking = THREE.BasicDepthPacking;
  ...
  const alphaMesh = boxMesh("alphaBody", new THREE.MeshBasicMaterial({ color: 0x633318 }));
  const sharedAlpha = alphaMesh.material;
  alphaMesh.material.index0AttributeName = "";
  const alphaTwin = boxMesh("alphaTwin", sharedAlpha);
  body.add(alphaMesh, alphaTwin);
  fastener.material.index0AttributeName = "color";
  const fastenerMat = fastener.material;
  ...
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  sharedGeoMat.version = 6;
  sharedGeoMat.index0AttributeName = "position";
  const sharedGeoVisual = new THREE.Mesh(mapped.geometry, sharedGeoMat);
  sharedGeoVisual.name = "sharedGeoBody";
  body.add(sharedGeoVisual);
  const shader = new THREE.ShaderMaterial();
  shader.index0AttributeName = "";
  const shaderMesh = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.05, 0.05), shader);
  shaderMesh.name = "shaderLike";
  body.add(shaderMesh);
  const colliderGrab = root.getObjectByName("collider_grab");
  colliderGrab.material.index0AttributeName = "position";
  const lidMesh = lid.getObjectByName("lidMesh");
  const latchMesh = latch.getObjectByName("latchMesh");
  lidMesh.material.index0AttributeName = "color";
  latchMesh.material.index0AttributeName = "";
  ...
  assert.equal(root.userData.lod, undefined, "fail-soft still does not invent lod groups");
  assert.equal(bodyMesh.material, bodyMat, "fail-soft does not replace the body material");
  assert.equal(materialIndex0AttributeNameAbsent(bodyMesh.material), true, "fail-soft body index0AttributeName is deleted");
  assert.equal(materialDepthPackingAbsent(bodyMesh.material), true, "fail-soft depthPacking pin still runs");
  assert.equal(materialExtensionsAbsent(bodyMesh.material), true, "fail-soft extensions pin still runs");
  assert.equal(bodyMesh.geometry.indirect, null, "fail-soft indirect pin still runs");
  assert.equal(bodyMesh.name, "lidMesh", "fail-soft reserved lidMesh name stays");
  assert.equal(bodyMesh.userData, bodyBag, "fail-soft mesh.userData identity stays when already empty");
  assert.equal(bodyMesh.matrixWorldNeedsUpdate, false, "fail-soft matrixWorldNeedsUpdate stays false");
  assert.equal(bodyMesh.material.version, 0, "fail-soft material.version pin still runs");
  assert.equal(materialIndex0AttributeNameAbsent(alphaMesh.material), true, "fail-soft shared color-only index0AttributeName is deleted once");
  assert.equal(alphaTwin.material, sharedAlpha, "fail-soft second mesh keeps the shared material");
  assert.equal(materialIndex0AttributeNameAbsent(fastener.material), true, "fail-soft fastener index0AttributeName is deleted");
  assert.equal(fastener.material, fastenerMat, "fail-soft does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fail-soft fastenerMesh stays named");
  assert.equal(materialIndex0AttributeNameAbsent(lidMesh.material), true, "fail-soft lidMesh index0AttributeName is deleted");
  assert.equal(lidMesh.name, "lidMesh", "fail-soft authored lidMesh name stays");
  assert.equal(materialIndex0AttributeNameAbsent(latchMesh.material), true, "fail-soft latchMesh index0AttributeName is deleted");
  assert.equal(latchMesh.name, "latchMesh", "fail-soft latchMesh name stays");
  assert.equal(mapped.material.index0AttributeName, "uv", "fail-soft mapped index0AttributeName stays authored");
  assert.equal(sharedGeoVisual.material.index0AttributeName, "position", "fail-soft geometry shared with a mapped mesh keeps index0AttributeName");
  assert.equal(sharedGeoMat.version, 6, "fail-soft shared material.version stays");
  assert.equal(shader.index0AttributeName, "", "fail-soft ShaderMaterial index0AttributeName stays authored");
  assert.equal(Object.hasOwn(shader, "index0AttributeName"), true, "fail-soft ShaderMaterial own property stays");
  assert.equal(colliderGrab.material.index0AttributeName, "position", "fail-soft collider index0AttributeName stays");
  assert.equal(colliderGrab.name, "collider_grab", "fail-soft collider name stays");
});

function materialDefaultAttributeValuesAbsent(material) {
  return material?.defaultAttributeValues === undefined && Object.hasOwn(material, "defaultAttributeValues") === false;
}

function shaderStyleDefaultAttributeValues() {
  return { color: [1, 1, 1], uv: [0, 0], uv1: [0, 0] };
}

test("packaged ingest deletes leftover Material defaultAttributeValues on color-only visuals and the fastener", () => {
  const { root, groups, fastener, lid, latch } = makePackagedFixture();
  const bodyMesh = groups[0][0].children.find((o) => o.isMesh);
  const bodyExtensions = { multiDraw: true };
  bodyMesh.material.extensions = bodyExtensions;
  bodyMesh.material.depthPacking = THREE.BasicDepthPacking;
  bodyMesh.material.index0AttributeName = "position";
  const bodyDefaults = shaderStyleDefaultAttributeValues();
  bodyMesh.material.defaultAttributeValues = bodyDefaults;
  const bodyIndirect = { label: "body-indirect" };
  bodyMesh.geometry.setIndirect(bodyIndirect);
  const bodyMat = bodyMesh.material;
  bodyMat.version = 4;
  fastener.material = new THREE.MeshBasicMaterial({ color: 0xbe7e31 });
  fastener.material.defaultAttributeValues = {};
  const fastenerMat = fastener.material;
  const fastenerIndirect = { label: "fastener-indirect" };
  fastener.geometry.setIndirect(fastenerIndirect);
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  const mappedDefaults = { uv: [0, 0] };
  mapped.material.defaultAttributeValues = mappedDefaults;
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  const sharedGeoDefaults = { color: [1, 1, 1] };
  sharedGeoMat.defaultAttributeValues = sharedGeoDefaults;
  const sharedGeoVisual = new THREE.Mesh(mapped.geometry, sharedGeoMat);
  sharedGeoVisual.name = "sharedGeoBody";
  const interleavedGeo = new THREE.BufferGeometry();
  const interleavedBuffer = new THREE.InterleavedBuffer(new Float32Array([0, 0, 0, 1, 0, 0]), 3);
  interleavedGeo.setAttribute("position", new THREE.InterleavedBufferAttribute(interleavedBuffer, 3, 0));
  const interleavedMat = new THREE.MeshBasicMaterial({ color: 0x633318 });
  const interleavedDefaults = shaderStyleDefaultAttributeValues();
  interleavedMat.defaultAttributeValues = interleavedDefaults;
  const interleaved = new THREE.Mesh(interleavedGeo, interleavedMat);
  interleaved.name = "interleavedMesh";
  const shader = new THREE.ShaderMaterial();
  const shaderDefaults = shader.defaultAttributeValues;
  const shaderMesh = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.1, 0.1), shader);
  shaderMesh.name = "shaderLike";
  const lit = boxMesh("litMesh", new THREE.MeshStandardMaterial());
  const litDefaults = { uv: [1, 1] };
  lit.material.defaultAttributeValues = litDefaults;
  groups[0][0].add(mapped, interleaved, shaderMesh, lit);
  root.add(sharedGeoVisual);
  const colliderGrab = root.getObjectByName("collider_grab");
  const colliderDefaults = {};
  colliderGrab.material.defaultAttributeValues = colliderDefaults;
  const lidMesh = lid.getObjectByName("lidL0");
  const latchMesh = latch.getObjectByName("latchL0");
  lidMesh.material.defaultAttributeValues = shaderStyleDefaultAttributeValues();
  latchMesh.material.defaultAttributeValues = {};
  lidMesh.name = "lidMesh";
  latchMesh.name = "latchMesh";
  ...
  assert.equal(bodyMesh.material, bodyMat, "ingest does not replace the body material");
  assert.equal(bodyMesh.material.isMeshBasicMaterial, true, "body stays MeshBasic");
  assert.equal(materialDefaultAttributeValuesAbsent(bodyMesh.material), true, "in-place color-only body defaultAttributeValues is deleted");
  assert.deepEqual(bodyDefaults, shaderStyleDefaultAttributeValues(), "pin does not mutate the detached defaultAttributeValues map");
  assert.equal(materialIndex0AttributeNameAbsent(bodyMesh.material), true, "index0AttributeName pin still clears leftover index0AttributeName");
  assert.equal(materialDepthPackingAbsent(bodyMesh.material), true, "depthPacking pin still clears leftover depthPacking");
  assert.equal(materialExtensionsAbsent(bodyMesh.material), true, "extensions pin still clears leftover extensions");
  assert.equal(bodyExtensions.multiDraw, true, "pin does not mutate the detached extensions object");
  assert.equal(bodyMesh.geometry.indirect, null, "indirect pin still runs before the defaultAttributeValues pin");
  assert.equal(bodyMesh.material.version, 0, "material.version pin still runs");
  assert.equal(unusedChannelAttributesAbsent(bodyMesh.geometry), true, "unused-channel strip still runs");
  assert.equal(materialDefaultAttributeValuesAbsent(fastener.material), true, "fastener defaultAttributeValues is deleted");
  assert.equal(fastener.geometry.indirect, null, "fastener indirect is still cleared");
  assert.equal(fastener.material, fastenerMat, "ingest does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fastenerMesh stays named");
  assert.equal(materialDefaultAttributeValuesAbsent(lidMesh.material), true, "reserved lidMesh defaultAttributeValues is deleted");
  assert.equal(materialDefaultAttributeValuesAbsent(latchMesh.material), true, "reserved latchMesh defaultAttributeValues is deleted");
  assert.equal(lidMesh.name, "lidMesh", "reserved lidMesh name stays");
  assert.equal(latchMesh.name, "latchMesh", "reserved latchMesh name stays");
  assert.equal(mapped.material.defaultAttributeValues, mappedDefaults, "mapped defaultAttributeValues stays authored");
  assert.equal(sharedGeoVisual.material.defaultAttributeValues, sharedGeoDefaults, "geometry shared with a mapped mesh keeps defaultAttributeValues");
  assert.equal(Object.hasOwn(sharedGeoMat, "defaultAttributeValues"), true, "shared-geometry map stays an own property");
  assert.equal(sharedGeoVisual.name, "sharedGeoBody", "shared-geometry mesh name stays");
  assert.equal(interleaved.material.defaultAttributeValues, interleavedDefaults, "interleaved defaultAttributeValues stays authored");
  assert.equal(shader.defaultAttributeValues, shaderDefaults, "ShaderMaterial constructor defaultAttributeValues stays authored");
  assert.deepEqual(shader.defaultAttributeValues, shaderStyleDefaultAttributeValues(), "ShaderMaterial constructor map is unchanged");
  assert.equal(lit.material.defaultAttributeValues, litDefaults, "MeshStandard defaultAttributeValues stays authored");
  assert.equal(colliderGrab.material.defaultAttributeValues, colliderDefaults, "collider defaultAttributeValues stays authored");
  assert.equal(Object.hasOwn(colliderGrab.material, "defaultAttributeValues"), true, "collider own defaultAttributeValues stays");
  assert.equal(colliderGrab.name, "collider_grab", "collider mesh name stays");
});

test("packaged ingest without lod groups still deletes leftover Material defaultAttributeValues", () => {
  const { root, body, lid, latch, fastener } = makePackagedFixture({ withLod: false });
  const bodyMesh = visualMeshes(body)[0];
  bodyMesh.material.defaultAttributeValues = shaderStyleDefaultAttributeValues();
  bodyMesh.material.index0AttributeName = "position";
  bodyMesh.material.depthPacking = THREE.BasicDepthPacking;
  bodyMesh.material.extensions = { multiDraw: true, clipCullDistance: true };
  const bodyIndirect = { label: "fail-soft-body-indirect" };
  ...
  const alphaMesh = boxMesh("alphaBody", new THREE.MeshBasicMaterial({ color: 0x633318 }));
  const sharedAlpha = alphaMesh.material;
  alphaMesh.material.defaultAttributeValues = {};
  const alphaTwin = boxMesh("alphaTwin", sharedAlpha);
  body.add(alphaMesh, alphaTwin);
  fastener.material.defaultAttributeValues = { uv: [0, 0] };
  const fastenerMat = fastener.material;
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  const mappedDefaults = shaderStyleDefaultAttributeValues();
  mapped.material.defaultAttributeValues = mappedDefaults;
  body.add(mapped);
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  sharedGeoMat.version = 6;
  const sharedGeoDefaults = { color: [0, 0, 0] };
  sharedGeoMat.defaultAttributeValues = sharedGeoDefaults;
  const sharedGeoVisual = new THREE.Mesh(mapped.geometry, sharedGeoMat);
  sharedGeoVisual.name = "sharedGeoBody";
  body.add(sharedGeoVisual);
  const shader = new THREE.ShaderMaterial();
  const shaderDefaults = shader.defaultAttributeValues;
  shader.defaultAttributeValues = { color: [1, 0, 0] };
  const authoredShader = shader.defaultAttributeValues;
  const shaderMesh = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.05, 0.05), shader);
  shaderMesh.name = "shaderLike";
  body.add(shaderMesh);
  const colliderGrab = root.getObjectByName("collider_grab");
  const colliderDefaults = shaderStyleDefaultAttributeValues();
  colliderGrab.material.defaultAttributeValues = colliderDefaults;
  const lidMesh = lid.getObjectByName("lidMesh");
  const latchMesh = latch.getObjectByName("latchMesh");
  lidMesh.material.defaultAttributeValues = { uv1: [0, 0] };
  latchMesh.material.defaultAttributeValues = {};
  ...
  assert.equal(root.userData.lod, undefined, "fail-soft still does not invent lod groups");
  assert.equal(bodyMesh.material, bodyMat, "fail-soft does not replace the body material");
  assert.equal(materialDefaultAttributeValuesAbsent(bodyMesh.material), true, "fail-soft body defaultAttributeValues is deleted");
  assert.equal(materialIndex0AttributeNameAbsent(bodyMesh.material), true, "fail-soft index0AttributeName pin still runs");
  assert.equal(materialDepthPackingAbsent(bodyMesh.material), true, "fail-soft depthPacking pin still runs");
  assert.equal(materialExtensionsAbsent(bodyMesh.material), true, "fail-soft extensions pin still runs");
  assert.equal(bodyMesh.geometry.indirect, null, "fail-soft indirect pin still runs");
  assert.equal(bodyMesh.name, "lidMesh", "fail-soft reserved lidMesh name stays");
  ...
  assert.equal(bodyMesh.matrixWorldNeedsUpdate, false, "fail-soft matrixWorldNeedsUpdate stays false");
  assert.equal(bodyMesh.material.version, 0, "fail-soft material.version pin still runs");
  assert.equal(materialDefaultAttributeValuesAbsent(alphaMesh.material), true, "fail-soft shared color-only defaultAttributeValues is deleted once");
  assert.equal(alphaTwin.material, sharedAlpha, "fail-soft second mesh keeps the shared material");
  assert.equal(materialDefaultAttributeValuesAbsent(fastener.material), true, "fail-soft fastener defaultAttributeValues is deleted");
  assert.equal(fastener.material, fastenerMat, "fail-soft does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fail-soft fastenerMesh stays named");
  assert.equal(materialDefaultAttributeValuesAbsent(lidMesh.material), true, "fail-soft lidMesh defaultAttributeValues is deleted");
  assert.equal(lidMesh.name, "lidMesh", "fail-soft authored lidMesh name stays");
  assert.equal(materialDefaultAttributeValuesAbsent(latchMesh.material), true, "fail-soft latchMesh defaultAttributeValues is deleted");
  assert.equal(latchMesh.name, "latchMesh", "fail-soft latchMesh name stays");
  assert.equal(mapped.material.defaultAttributeValues, mappedDefaults, "fail-soft mapped defaultAttributeValues stays authored");
  assert.equal(sharedGeoVisual.material.defaultAttributeValues, sharedGeoDefaults, "fail-soft geometry shared with a mapped mesh keeps defaultAttributeValues");
  assert.equal(sharedGeoMat.version, 6, "fail-soft shared material.version stays");
  assert.equal(shader.defaultAttributeValues, authoredShader, "fail-soft ShaderMaterial defaultAttributeValues stays authored");
  assert.notEqual(shader.defaultAttributeValues, shaderDefaults, "fail-soft fixture overwrote the constructor map");
  assert.equal(Object.hasOwn(shader, "defaultAttributeValues"), true, "fail-soft ShaderMaterial own property stays");
  assert.equal(colliderGrab.material.defaultAttributeValues, colliderDefaults, "fail-soft collider defaultAttributeValues stays");
  assert.equal(colliderGrab.name, "collider_grab", "fail-soft collider name stays");
});
...
test("packaged ingest deletes leftover Material uniforms on color-only visuals and the fastener", () => {
  const { root, groups, fastener, lid, latch } = makePackagedFixture();
  const bodyMesh = groups[0][0].children.find((o) => o.isMesh);
  const bodyExtensions = { multiDraw: true };
  bodyMesh.material.extensions = bodyExtensions;
  ...
  bodyMesh.material.index0AttributeName = "position";
  bodyMesh.material.defaultAttributeValues = shaderStyleDefaultAttributeValues();
  const bodyUniforms = shaderStyleUniformMap();
  bodyMesh.material.uniforms = bodyUniforms;
  const bodyIndirect = { label: "body-indirect" };
  bodyMesh.geometry.setIndirect(bodyIndirect);
  const bodyMat = bodyMesh.material;
  bodyMat.version = 4;
  fastener.material = new THREE.MeshBasicMaterial({ color: 0xbe7e31 });
  fastener.material.uniforms = {};
  const fastenerMat = fastener.material;
  const fastenerIndirect = { label: "fastener-indirect" };
  fastener.geometry.setIndirect(fastenerIndirect);
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  const mappedUniforms = { uv: { value: new THREE.Vector2(0, 0) } };
  mapped.material.uniforms = mappedUniforms;
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  const sharedGeoUniforms = { color: { value: new THREE.Color(1, 1, 1) } };
  ...
  interleavedGeo.setAttribute("position", new THREE.InterleavedBufferAttribute(interleavedBuffer, 3, 0));
  const interleavedMat = new THREE.MeshBasicMaterial({ color: 0x633318 });
  const interleavedUniforms = shaderStyleUniformMap();
  ...
  const rawUniforms = raw.uniforms;
  const rawMesh = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.1, 0.1), raw);
  rawMesh.name = "rawLike";
  const lit = boxMesh("litMesh", new THREE.MeshStandardMaterial());
  const litUniforms = { uv: { value: new THREE.Vector2(1, 1) } };
  lit.material.uniforms = litUniforms;
  groups[0][0].add(mapped, interleaved, shaderMesh, rawMesh, lit);
  root.add(sharedGeoVisual);
  const colliderGrab = root.getObjectByName("collider_grab");
  const colliderUniforms = {};
  colliderGrab.material.uniforms = colliderUniforms;
  const lidMesh = lid.getObjectByName("lidL0");
  const latchMesh = latch.getObjectByName("latchL0");
  lidMesh.material.uniforms = shaderStyleUniformMap();
  latchMesh.material.uniforms = {};
  lidMesh.name = "lidMesh";
  latchMesh.name = "latchMesh";
  ...
  assert.equal(bodyMesh.material, bodyMat, "ingest does not replace the body material");
  assert.equal(bodyMesh.material.isMeshBasicMaterial, true, "body stays MeshBasic");
  assert.equal(materialUniformsAbsent(bodyMesh.material), true, "in-place color-only body uniforms is deleted");
  assert.equal(bodyUniforms.diffuse.value.getHex(), 0x633318, "pin does not mutate the detached uniforms Color");
  assert.equal(materialDefaultAttributeValuesAbsent(bodyMesh.material), true, "defaultAttributeValues pin still clears leftover defaultAttributeValues");
  assert.equal(materialIndex0AttributeNameAbsent(bodyMesh.material), true, "index0AttributeName pin still clears leftover index0AttributeName");
  assert.equal(materialDepthPackingAbsent(bodyMesh.material), true, "depthPacking pin still clears leftover depthPacking");
  assert.equal(materialExtensionsAbsent(bodyMesh.material), true, "extensions pin still clears leftover extensions");
  assert.equal(bodyExtensions.multiDraw, true, "pin does not mutate the detached extensions object");
  assert.equal(bodyMesh.geometry.indirect, null, "indirect pin still runs before the uniforms pin");
  assert.equal(bodyMesh.material.version, 0, "material.version pin still runs");
  assert.equal(unusedChannelAttributesAbsent(bodyMesh.geometry), true, "unused-channel strip still runs");
  assert.equal(materialUniformsAbsent(fastener.material), true, "fastener uniforms is deleted");
  assert.equal(fastener.geometry.indirect, null, "fastener indirect is still cleared");
  assert.equal(fastener.material, fastenerMat, "ingest does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fastenerMesh stays named");
  assert.equal(materialUniformsAbsent(lidMesh.material), true, "reserved lidMesh uniforms is deleted");
  assert.equal(materialUniformsAbsent(latchMesh.material), true, "reserved latchMesh uniforms is deleted");
  assert.equal(lidMesh.name, "lidMesh", "reserved lidMesh name stays");
  assert.equal(latchMesh.name, "latchMesh", "reserved latchMesh name stays");
  assert.equal(mapped.material.uniforms, mappedUniforms, "mapped uniforms stays authored");
  ...
  assert.equal(sharedGeoVisual.name, "sharedGeoBody", "shared-geometry mesh name stays");
  assert.equal(interleaved.material.uniforms, interleavedUniforms, "interleaved uniforms stays authored");
  assert.equal(shader.uniforms, shaderUniforms, "ShaderMaterial constructor uniforms stays authored");
  assert.deepEqual(shader.uniforms, {}, "ShaderMaterial constructor map is unchanged");
  assert.equal(raw.uniforms, rawUniforms, "RawShaderMaterial constructor uniforms stays authored");
  assert.equal(raw.isRawShaderMaterial, true, "RawShaderMaterial stays a RawShaderMaterial");
  assert.equal(lit.material.uniforms, litUniforms, "MeshStandard uniforms stays authored");
  assert.equal(colliderGrab.material.uniforms, colliderUniforms, "collider uniforms stays authored");
  assert.equal(Object.hasOwn(colliderGrab.material, "uniforms"), true, "collider own uniforms stays");
  assert.equal(colliderGrab.name, "collider_grab", "collider mesh name stays");
});

test("packaged ingest without lod groups still deletes leftover Material uniforms", () => {
  const { root, body, lid, latch, fastener } = makePackagedFixture({ withLod: false });
  const bodyMesh = visualMeshes(body)[0];
  bodyMesh.material.uniforms = shaderStyleUniformMap();
  bodyMesh.material.defaultAttributeValues = shaderStyleDefaultAttributeValues();
  bodyMesh.material.index0AttributeName = "position";
  bodyMesh.material.depthPacking = THREE.BasicDepthPacking;
  bodyMesh.material.extensions = { multiDraw: true, clipCullDistance: true };
  ...
  const alphaMesh = boxMesh("alphaBody", new THREE.MeshBasicMaterial({ color: 0x633318 }));
  const sharedAlpha = alphaMesh.material;
  alphaMesh.material.uniforms = {};
  const alphaTwin = boxMesh("alphaTwin", sharedAlpha);
  body.add(alphaMesh, alphaTwin);
  fastener.material.uniforms = { uv: { value: new THREE.Vector2(0, 0) } };
  const fastenerMat = fastener.material;
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  const mappedUniforms = shaderStyleUniformMap();
  mapped.material.uniforms = mappedUniforms;
  body.add(mapped);
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  sharedGeoMat.version = 6;
  const sharedGeoUniforms = { color: { value: new THREE.Color(0, 0, 0) } };
  sharedGeoMat.uniforms = sharedGeoUniforms;
  const sharedGeoVisual = new THREE.Mesh(mapped.geometry, sharedGeoMat);
  sharedGeoVisual.name = "sharedGeoBody";
  body.add(sharedGeoVisual);
  const shader = new THREE.ShaderMaterial();
  const shaderDefaults = shader.uniforms;
  shader.uniforms = { tint: { value: new THREE.Color(1, 0, 0) } };
  const authoredShader = shader.uniforms;
  const shaderMesh = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.05, 0.05), shader);
  shaderMesh.name = "shaderLike";
  body.add(shaderMesh);
  const raw = new THREE.RawShaderMaterial();
  raw.uniforms = { tint: { value: new THREE.Vector3(1, 0, 0) } };
  const authoredRaw = raw.uniforms;
  const rawMesh = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.05, 0.05), raw);
  rawMesh.name = "rawLike";
  body.add(rawMesh);
  const colliderGrab = root.getObjectByName("collider_grab");
  ...
  const lidMesh = lid.getObjectByName("lidMesh");
  const latchMesh = latch.getObjectByName("latchMesh");
  lidMesh.material.uniforms = { uv1: { value: new THREE.Vector2(0, 0) } };
  latchMesh.material.uniforms = {};
  ...
  assert.equal(root.userData.lod, undefined, "fail-soft still does not invent lod groups");
  assert.equal(bodyMesh.material, bodyMat, "fail-soft does not replace the body material");
  assert.equal(materialUniformsAbsent(bodyMesh.material), true, "fail-soft body uniforms is deleted");
  assert.equal(materialDefaultAttributeValuesAbsent(bodyMesh.material), true, "fail-soft defaultAttributeValues pin still runs");
  assert.equal(materialIndex0AttributeNameAbsent(bodyMesh.material), true, "fail-soft index0AttributeName pin still runs");
  assert.equal(materialDepthPackingAbsent(bodyMesh.material), true, "fail-soft depthPacking pin still runs");
  assert.equal(materialExtensionsAbsent(bodyMesh.material), true, "fail-soft extensions pin still runs");
  assert.equal(bodyMesh.geometry.indirect, null, "fail-soft indirect pin still runs");
  ...
  assert.equal(bodyMesh.matrixWorldNeedsUpdate, false, "fail-soft matrixWorldNeedsUpdate stays false");
  assert.equal(bodyMesh.material.version, 0, "fail-soft material.version pin still runs");
  assert.equal(materialUniformsAbsent(alphaMesh.material), true, "fail-soft shared color-only uniforms is deleted once");
  assert.equal(alphaTwin.material, sharedAlpha, "fail-soft second mesh keeps the shared material");
  assert.equal(materialUniformsAbsent(fastener.material), true, "fail-soft fastener uniforms is deleted");
  assert.equal(fastener.material, fastenerMat, "fail-soft does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fail-soft fastenerMesh stays named");
  assert.equal(materialUniformsAbsent(lidMesh.material), true, "fail-soft lidMesh uniforms is deleted");
  assert.equal(lidMesh.name, "lidMesh", "fail-soft authored lidMesh name stays");
  assert.equal(materialUniformsAbsent(latchMesh.material), true, "fail-soft latchMesh uniforms is deleted");
  assert.equal(latchMesh.name, "latchMesh", "fail-soft latchMesh name stays");
  assert.equal(mapped.material.uniforms, mappedUniforms, "fail-soft mapped uniforms stays authored");
  assert.equal(sharedGeoVisual.material.uniforms, sharedGeoUniforms, "fail-soft geometry shared with a mapped mesh keeps uniforms");
  assert.equal(sharedGeoMat.version, 6, "fail-soft shared material.version stays");
  assert.equal(shader.uniforms, authoredShader, "fail-soft ShaderMaterial uniforms stays authored");
  assert.notEqual(shader.uniforms, shaderDefaults, "fail-soft fixture overwrote the constructor map");
  assert.equal(Object.hasOwn(shader, "uniforms"), true, "fail-soft ShaderMaterial own property stays");
  assert.equal(raw.uniforms, authoredRaw, "fail-soft RawShaderMaterial uniforms stays authored");
  assert.equal(Object.hasOwn(raw, "uniforms"), true, "fail-soft RawShaderMaterial own property stays");
  assert.equal(colliderGrab.material.uniforms, colliderUniforms, "fail-soft collider uniforms stays");
  assert.equal(colliderGrab.name, "collider_grab", "fail-soft collider name stays");
});

function materialUniformsNeedUpdateAbsent(material) {
  return material?.uniformsNeedUpdate === undefined && Object.hasOwn(material, "uniformsNeedUpdate") === false;
}

test("packaged ingest deletes leftover Material uniformsNeedUpdate on color-only visuals and the fastener", () => {
  const { root, groups, fastener, lid, latch } = makePackagedFixture();
  const bodyMesh = groups[0][0].children.find((o) => o.isMesh);
  const bodyExtensions = { multiDraw: true };
  bodyMesh.material.extensions = bodyExtensions;
  ...
  const bodyUniforms = shaderStyleUniformMap();
  bodyMesh.material.uniforms = bodyUniforms;
  bodyMesh.material.uniformsNeedUpdate = true;
  const bodyIndirect = { label: "body-indirect" };
  bodyMesh.geometry.setIndirect(bodyIndirect);
  const bodyMat = bodyMesh.material;
  bodyMat.version = 4;
  fastener.material = new THREE.MeshBasicMaterial({ color: 0xbe7e31 });
  fastener.material.uniformsNeedUpdate = false;
  const fastenerMat = fastener.material;
  const fastenerIndirect = { label: "fastener-indirect" };
  fastener.geometry.setIndirect(fastenerIndirect);
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  mapped.material.uniformsNeedUpdate = true;
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  sharedGeoMat.uniformsNeedUpdate = false;
  const sharedGeoVisual = new THREE.Mesh(mapped.geometry, sharedGeoMat);
  sharedGeoVisual.name = "sharedGeoBody";
  const interleavedGeo = new THREE.BufferGeometry();
  const interleavedBuffer = new THREE.InterleavedBuffer(new Float32Array([0, 0, 0, 1, 0, 0]), 3);
  interleavedGeo.setAttribute("position", new THREE.InterleavedBufferAttribute(interleavedBuffer, 3, 0));
  const interleavedMat = new THREE.MeshBasicMaterial({ color: 0x633318 });
  interleavedMat.uniformsNeedUpdate = true;
  const interleaved = new THREE.Mesh(interleavedGeo, interleavedMat);
  interleaved.name = "interleavedMesh";
  const shader = new THREE.ShaderMaterial();
  ...
  shaderMesh.name = "shaderLike";
  const raw = new THREE.RawShaderMaterial();
  raw.uniformsNeedUpdate = true;
  const rawMesh = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.1, 0.1), raw);
  rawMesh.name = "rawLike";
  const lit = boxMesh("litMesh", new THREE.MeshStandardMaterial());
  lit.material.uniformsNeedUpdate = false;
  groups[0][0].add(mapped, interleaved, shaderMesh, rawMesh, lit);
  root.add(sharedGeoVisual);
  const colliderGrab = root.getObjectByName("collider_grab");
  colliderGrab.material.uniformsNeedUpdate = true;
  const lidMesh = lid.getObjectByName("lidL0");
  const latchMesh = latch.getObjectByName("latchL0");
  lidMesh.material.uniformsNeedUpdate = true;
  latchMesh.material.uniformsNeedUpdate = false;
  lidMesh.name = "lidMesh";
  latchMesh.name = "latchMesh";
  ...
  assert.equal(bodyMesh.material, bodyMat, "ingest does not replace the body material");
  assert.equal(bodyMesh.material.isMeshBasicMaterial, true, "body stays MeshBasic");
  assert.equal(materialUniformsNeedUpdateAbsent(bodyMesh.material), true, "in-place color-only body uniformsNeedUpdate is deleted");
  assert.equal(materialUniformsAbsent(bodyMesh.material), true, "uniforms pin still clears leftover uniforms");
  assert.equal(bodyUniforms.diffuse.value.getHex(), 0x633318, "pin does not mutate the detached uniforms Color");
  assert.equal(materialDefaultAttributeValuesAbsent(bodyMesh.material), true, "defaultAttributeValues pin still clears leftover defaultAttributeValues");
  assert.equal(materialIndex0AttributeNameAbsent(bodyMesh.material), true, "index0AttributeName pin still clears leftover index0AttributeName");
  assert.equal(materialDepthPackingAbsent(bodyMesh.material), true, "depthPacking pin still clears leftover depthPacking");
  assert.equal(materialExtensionsAbsent(bodyMesh.material), true, "extensions pin still clears leftover extensions");
  assert.equal(bodyExtensions.multiDraw, true, "pin does not mutate the detached extensions object");
  assert.equal(bodyMesh.geometry.indirect, null, "indirect pin still runs before the uniformsNeedUpdate pin");
  assert.equal(bodyMesh.material.version, 0, "material.version pin still runs");
  assert.equal(unusedChannelAttributesAbsent(bodyMesh.geometry), true, "unused-channel strip still runs");
  assert.equal(materialUniformsNeedUpdateAbsent(fastener.material), true, "fastener uniformsNeedUpdate is deleted");
  assert.equal(fastener.geometry.indirect, null, "fastener indirect is still cleared");
  assert.equal(fastener.material, fastenerMat, "ingest does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fastenerMesh stays named");
  assert.equal(materialUniformsNeedUpdateAbsent(lidMesh.material), true, "reserved lidMesh uniformsNeedUpdate is deleted");
  assert.equal(materialUniformsNeedUpdateAbsent(latchMesh.material), true, "reserved latchMesh uniformsNeedUpdate is deleted");
  assert.equal(lidMesh.name, "lidMesh", "reserved lidMesh name stays");
  assert.equal(latchMesh.name, "latchMesh", "reserved latchMesh name stays");
  assert.equal(mapped.material.uniformsNeedUpdate, true, "mapped uniformsNeedUpdate stays authored");
  assert.equal(sharedGeoVisual.material.uniformsNeedUpdate, false, "geometry shared with a mapped mesh keeps uniformsNeedUpdate");
  assert.equal(Object.hasOwn(sharedGeoMat, "uniformsNeedUpdate"), true, "shared-geometry false stays an own property");
  assert.equal(sharedGeoVisual.name, "sharedGeoBody", "shared-geometry mesh name stays");
  assert.equal(interleaved.material.uniformsNeedUpdate, true, "interleaved uniformsNeedUpdate stays authored");
  assert.equal(shader.uniformsNeedUpdate, true, "ShaderMaterial authored uniformsNeedUpdate stays");
  assert.equal(Object.hasOwn(shader, "uniformsNeedUpdate"), true, "ShaderMaterial own property stays");
  assert.equal(raw.uniformsNeedUpdate, true, "RawShaderMaterial authored uniformsNeedUpdate stays");
  assert.equal(raw.isRawShaderMaterial, true, "RawShaderMaterial stays a RawShaderMaterial");
  assert.equal(lit.material.uniformsNeedUpdate, false, "MeshStandard uniformsNeedUpdate stays authored");
  assert.equal(colliderGrab.material.uniformsNeedUpdate, true, "collider uniformsNeedUpdate stays authored");
  assert.equal(Object.hasOwn(colliderGrab.material, "uniformsNeedUpdate"), true, "collider own uniformsNeedUpdate stays");
  assert.equal(colliderGrab.name, "collider_grab", "collider mesh name stays");
});

test("packaged ingest without lod groups still deletes leftover Material uniformsNeedUpdate", () => {
  const { root, body, lid, latch, fastener } = makePackagedFixture({ withLod: false });
  const bodyMesh = visualMeshes(body)[0];
  bodyMesh.material.uniformsNeedUpdate = true;
  bodyMesh.material.uniforms = shaderStyleUniformMap();
  ...
  const alphaMesh = boxMesh("alphaBody", new THREE.MeshBasicMaterial({ color: 0x633318 }));
  const sharedAlpha = alphaMesh.material;
  alphaMesh.material.uniformsNeedUpdate = false;
  const alphaTwin = boxMesh("alphaTwin", sharedAlpha);
  body.add(alphaMesh, alphaTwin);
  fastener.material.uniformsNeedUpdate = true;
  const fastenerMat = fastener.material;
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  mapped.material.uniformsNeedUpdate = null;
  body.add(mapped);
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  sharedGeoMat.version = 6;
  sharedGeoMat.uniformsNeedUpdate = true;
  const sharedGeoVisual = new THREE.Mesh(mapped.geometry, sharedGeoMat);
  sharedGeoVisual.name = "sharedGeoBody";
  body.add(sharedGeoVisual);
  const shader = new THREE.ShaderMaterial();
  ...
  body.add(shaderMesh);
  const raw = new THREE.RawShaderMaterial();
  raw.uniformsNeedUpdate = false;
  const rawMesh = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.05, 0.05), raw);
  rawMesh.name = "rawLike";
  body.add(rawMesh);
  const colliderGrab = root.getObjectByName("collider_grab");
  colliderGrab.material.uniformsNeedUpdate = false;
  const lidMesh = lid.getObjectByName("lidMesh");
  const latchMesh = latch.getObjectByName("latchMesh");
  lidMesh.material.uniformsNeedUpdate = true;
  latchMesh.material.uniformsNeedUpdate = undefined;

  ingestPackagedRoot(root, sidecar);

  assert.equal(root.userData.lod, undefined, "fail-soft still does not invent lod groups");
  assert.equal(bodyMesh.material, bodyMat, "fail-soft does not replace the body material");
  assert.equal(materialUniformsNeedUpdateAbsent(bodyMesh.material), true, "fail-soft body uniformsNeedUpdate is deleted");
  assert.equal(materialUniformsAbsent(bodyMesh.material), true, "fail-soft uniforms pin still runs");
  assert.equal(materialDefaultAttributeValuesAbsent(bodyMesh.material), true, "fail-soft defaultAttributeValues pin still runs");
  assert.equal(materialIndex0AttributeNameAbsent(bodyMesh.material), true, "fail-soft index0AttributeName pin still runs");
  assert.equal(materialDepthPackingAbsent(bodyMesh.material), true, "fail-soft depthPacking pin still runs");
  assert.equal(materialExtensionsAbsent(bodyMesh.material), true, "fail-soft extensions pin still runs");
  ...
  assert.equal(bodyMesh.matrixWorldNeedsUpdate, false, "fail-soft matrixWorldNeedsUpdate stays false");
  assert.equal(bodyMesh.material.version, 0, "fail-soft material.version pin still runs");
  assert.equal(materialUniformsNeedUpdateAbsent(alphaMesh.material), true, "fail-soft shared color-only uniformsNeedUpdate is deleted once");
  assert.equal(alphaTwin.material, sharedAlpha, "fail-soft second mesh keeps the shared material");
  assert.equal(materialUniformsNeedUpdateAbsent(fastener.material), true, "fail-soft fastener uniformsNeedUpdate is deleted");
  assert.equal(fastener.material, fastenerMat, "fail-soft does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fail-soft fastenerMesh stays named");
  assert.equal(materialUniformsNeedUpdateAbsent(lidMesh.material), true, "fail-soft lidMesh uniformsNeedUpdate is deleted");
  assert.equal(lidMesh.name, "lidMesh", "fail-soft authored lidMesh name stays");
  assert.equal(materialUniformsNeedUpdateAbsent(latchMesh.material), true, "fail-soft latchMesh own undefined is deleted");
  assert.equal(latchMesh.name, "latchMesh", "fail-soft latchMesh name stays");
  assert.equal(mapped.material.uniformsNeedUpdate, null, "fail-soft mapped uniformsNeedUpdate stays authored");
  assert.equal(sharedGeoVisual.material.uniformsNeedUpdate, true, "fail-soft geometry shared with a mapped mesh keeps uniformsNeedUpdate");
  assert.equal(sharedGeoMat.version, 6, "fail-soft shared material.version stays");
  assert.equal(shader.uniformsNeedUpdate, true, "fail-soft ShaderMaterial uniformsNeedUpdate stays authored");
  assert.equal(Object.hasOwn(shader, "uniformsNeedUpdate"), true, "fail-soft ShaderMaterial own property stays");
  assert.equal(raw.uniformsNeedUpdate, false, "fail-soft RawShaderMaterial uniformsNeedUpdate stays authored");
  assert.equal(Object.hasOwn(raw, "uniformsNeedUpdate"), true, "fail-soft RawShaderMaterial own property stays");
  assert.equal(colliderGrab.material.uniformsNeedUpdate, false, "fail-soft collider uniformsNeedUpdate stays");
  assert.equal(colliderGrab.name, "collider_grab", "fail-soft collider name stays");
});

function materialUniformsGroupsAbsent(material) {
  return material?.uniformsGroups === undefined && Object.hasOwn(material, "uniformsGroups") === false;
}

function shaderStyleUniformsGroup(name = "ubo") {
  return { id: name, name };
}

test("packaged ingest deletes leftover Material uniformsGroups on color-only visuals and the fastener", () => {
  const { root, groups, fastener, lid, latch } = makePackagedFixture();
  const bodyMesh = groups[0][0].children.find((o) => o.isMesh);
  const bodyExtensions = { multiDraw: true };
  bodyMesh.material.extensions = bodyExtensions;
  ...
  bodyMesh.material.uniforms = bodyUniforms;
  bodyMesh.material.uniformsNeedUpdate = true;
  const bodyGroups = [shaderStyleUniformsGroup("body")];
  bodyMesh.material.uniformsGroups = bodyGroups;
  const bodyIndirect = { label: "body-indirect" };
  bodyMesh.geometry.setIndirect(bodyIndirect);
  const bodyMat = bodyMesh.material;
  bodyMat.version = 4;
  fastener.material = new THREE.MeshBasicMaterial({ color: 0xbe7e31 });
  fastener.material.uniformsGroups = [];
  const fastenerMat = fastener.material;
  const fastenerIndirect = { label: "fastener-indirect" };
  fastener.geometry.setIndirect(fastenerIndirect);
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  const mappedGroups = [shaderStyleUniformsGroup("mapped")];
  mapped.material.uniformsGroups = mappedGroups;
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  sharedGeoMat.uniformsGroups = [];
  const sharedGeoVisual = new THREE.Mesh(mapped.geometry, sharedGeoMat);
  sharedGeoVisual.name = "sharedGeoBody";
  const interleavedGeo = new THREE.BufferGeometry();
  const interleavedBuffer = new THREE.InterleavedBuffer(new Float32Array([0, 0, 0, 1, 0, 0]), 3);
  interleavedGeo.setAttribute("position", new THREE.InterleavedBufferAttribute(interleavedBuffer, 3, 0));
  const interleavedMat = new THREE.MeshBasicMaterial({ color: 0x633318 });
  const interleavedGroups = [shaderStyleUniformsGroup("interleaved")];
  interleavedMat.uniformsGroups = interleavedGroups;
  const interleaved = new THREE.Mesh(interleavedGeo, interleavedMat);
  interleaved.name = "interleavedMesh";
  const shader = new THREE.ShaderMaterial();
  const authoredShader = [shaderStyleUniformsGroup("shader")];
  shader.uniformsGroups = authoredShader;
  const shaderMesh = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.1, 0.1), shader);
  shaderMesh.name = "shaderLike";
  const raw = new THREE.RawShaderMaterial();
  const authoredRaw = [shaderStyleUniformsGroup("raw")];
  raw.uniformsGroups = authoredRaw;
  const rawMesh = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.1, 0.1), raw);
  rawMesh.name = "rawLike";
  const lit = boxMesh("litMesh", new THREE.MeshStandardMaterial());
  const litGroups = {};
  lit.material.uniformsGroups = litGroups;
  groups[0][0].add(mapped, interleaved, shaderMesh, rawMesh, lit);
  root.add(sharedGeoVisual);
  const colliderGrab = root.getObjectByName("collider_grab");
  const colliderGroups = [shaderStyleUniformsGroup("collider")];
  colliderGrab.material.uniformsGroups = colliderGroups;
  const lidMesh = lid.getObjectByName("lidL0");
  const latchMesh = latch.getObjectByName("latchL0");
  lidMesh.material.uniformsGroups = [shaderStyleUniformsGroup("lid")];
  latchMesh.material.uniformsGroups = null;
  lidMesh.name = "lidMesh";
  latchMesh.name = "latchMesh";
  ...
  assert.equal(bodyMesh.material, bodyMat, "ingest does not replace the body material");
  assert.equal(bodyMesh.material.isMeshBasicMaterial, true, "body stays MeshBasic");
  assert.equal(materialUniformsGroupsAbsent(bodyMesh.material), true, "in-place color-only body uniformsGroups is deleted");
  assert.equal(bodyGroups[0].name, "body", "pin does not mutate the detached group");
  assert.equal(materialUniformsNeedUpdateAbsent(bodyMesh.material), true, "uniformsNeedUpdate pin still clears leftover uniformsNeedUpdate");
  assert.equal(materialUniformsAbsent(bodyMesh.material), true, "uniforms pin still clears leftover uniforms");
  assert.equal(bodyUniforms.diffuse.value.getHex(), 0x633318, "pin does not mutate the detached uniforms Color");
  assert.equal(materialDefaultAttributeValuesAbsent(bodyMesh.material), true, "defaultAttributeValues pin still clears leftover defaultAttributeValues");
  assert.equal(materialIndex0AttributeNameAbsent(bodyMesh.material), true, "index0AttributeName pin still clears leftover index0AttributeName");
  ...
  assert.equal(materialExtensionsAbsent(bodyMesh.material), true, "extensions pin still clears leftover extensions");
  assert.equal(bodyExtensions.multiDraw, true, "pin does not mutate the detached extensions object");
  assert.equal(bodyMesh.geometry.indirect, null, "indirect pin still runs before the uniformsGroups pin");
  assert.equal(bodyMesh.material.version, 0, "material.version pin still runs");
  assert.equal(unusedChannelAttributesAbsent(bodyMesh.geometry), true, "unused-channel strip still runs");
  assert.equal(materialUniformsGroupsAbsent(fastener.material), true, "fastener uniformsGroups is deleted");
  assert.equal(fastener.geometry.indirect, null, "fastener indirect is still cleared");
  assert.equal(fastener.material, fastenerMat, "ingest does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fastenerMesh stays named");
  assert.equal(materialUniformsGroupsAbsent(lidMesh.material), true, "reserved lidMesh uniformsGroups is deleted");
  assert.equal(materialUniformsGroupsAbsent(latchMesh.material), true, "reserved latchMesh uniformsGroups is deleted");
  assert.equal(lidMesh.name, "lidMesh", "reserved lidMesh name stays");
  assert.equal(latchMesh.name, "latchMesh", "reserved latchMesh name stays");
  assert.equal(mapped.material.uniformsGroups, mappedGroups, "mapped uniformsGroups stays authored");
  assert.equal(sharedGeoVisual.material.uniformsGroups, sharedGeoMat.uniformsGroups, "geometry shared with a mapped mesh keeps uniformsGroups");
  assert.equal(Object.hasOwn(sharedGeoMat, "uniformsGroups"), true, "shared-geometry [] stays an own property");
  assert.equal(sharedGeoVisual.material.uniformsGroups.length, 0, "shared-geometry empty array stays");
  assert.equal(sharedGeoVisual.name, "sharedGeoBody", "shared-geometry mesh name stays");
  assert.equal(interleaved.material.uniformsGroups, interleavedGroups, "interleaved uniformsGroups stays authored");
  assert.equal(shader.uniformsGroups, authoredShader, "ShaderMaterial authored uniformsGroups stays");
  assert.equal(Object.hasOwn(shader, "uniformsGroups"), true, "ShaderMaterial own property stays");
  assert.equal(raw.uniformsGroups, authoredRaw, "RawShaderMaterial authored uniformsGroups stays");
  assert.equal(raw.isRawShaderMaterial, true, "RawShaderMaterial stays a RawShaderMaterial");
  assert.equal(lit.material.uniformsGroups, litGroups, "MeshStandard uniformsGroups stays authored");
  assert.equal(colliderGrab.material.uniformsGroups, colliderGroups, "collider uniformsGroups stays authored");
  assert.equal(Object.hasOwn(colliderGrab.material, "uniformsGroups"), true, "collider own uniformsGroups stays");
  assert.equal(colliderGrab.name, "collider_grab", "collider mesh name stays");
});

test("packaged ingest without lod groups still deletes leftover Material uniformsGroups", () => {
  const { root, body, lid, latch, fastener } = makePackagedFixture({ withLod: false });
  const bodyMesh = visualMeshes(body)[0];
  bodyMesh.material.uniformsGroups = [shaderStyleUniformsGroup("fail-soft")];
  bodyMesh.material.uniformsNeedUpdate = true;
  bodyMesh.material.uniforms = shaderStyleUniformMap();
  bodyMesh.material.defaultAttributeValues = shaderStyleDefaultAttributeValues();
  bodyMesh.material.index0AttributeName = "position";
  ...
  const alphaMesh = boxMesh("alphaBody", new THREE.MeshBasicMaterial({ color: 0x633318 }));
  const sharedAlpha = alphaMesh.material;
  alphaMesh.material.uniformsGroups = [];
  const alphaTwin = boxMesh("alphaTwin", sharedAlpha);
  body.add(alphaMesh, alphaTwin);
  fastener.material.uniformsGroups = {};
  const fastenerMat = fastener.material;
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  mapped.material.uniformsGroups = null;
  body.add(mapped);
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  sharedGeoMat.version = 6;
  const sharedGeoGroups = [shaderStyleUniformsGroup("shared-geo")];
  sharedGeoMat.uniformsGroups = sharedGeoGroups;
  const sharedGeoVisual = new THREE.Mesh(mapped.geometry, sharedGeoMat);
  sharedGeoVisual.name = "sharedGeoBody";
  body.add(sharedGeoVisual);
  const shader = new THREE.ShaderMaterial();
  ...
  body.add(rawMesh);
  const colliderGrab = root.getObjectByName("collider_grab");
  colliderGrab.material.uniformsGroups = {};
  const lidMesh = lid.getObjectByName("lidMesh");
  const latchMesh = latch.getObjectByName("latchMesh");
  lidMesh.material.uniformsGroups = [shaderStyleUniformsGroup("lid")];
  latchMesh.material.uniformsGroups = undefined;

  ingestPackagedRoot(root, sidecar);

  assert.equal(root.userData.lod, undefined, "fail-soft still does not invent lod groups");
  assert.equal(bodyMesh.material, bodyMat, "fail-soft does not replace the body material");
  assert.equal(materialUniformsGroupsAbsent(bodyMesh.material), true, "fail-soft body uniformsGroups is deleted");
  assert.equal(materialUniformsNeedUpdateAbsent(bodyMesh.material), true, "fail-soft uniformsNeedUpdate pin still runs");
  assert.equal(materialUniformsAbsent(bodyMesh.material), true, "fail-soft uniforms pin still runs");
  assert.equal(materialDefaultAttributeValuesAbsent(bodyMesh.material), true, "fail-soft defaultAttributeValues pin still runs");
  assert.equal(materialIndex0AttributeNameAbsent(bodyMesh.material), true, "fail-soft index0AttributeName pin still runs");
  assert.equal(materialDepthPackingAbsent(bodyMesh.material), true, "fail-soft depthPacking pin still runs");
  ...
  assert.equal(bodyMesh.matrixWorldNeedsUpdate, false, "fail-soft matrixWorldNeedsUpdate stays false");
  assert.equal(bodyMesh.material.version, 0, "fail-soft material.version pin still runs");
  assert.equal(materialUniformsGroupsAbsent(alphaMesh.material), true, "fail-soft shared color-only uniformsGroups is deleted once");
  assert.equal(alphaTwin.material, sharedAlpha, "fail-soft second mesh keeps the shared material");
  assert.equal(materialUniformsGroupsAbsent(fastener.material), true, "fail-soft fastener uniformsGroups is deleted");
  assert.equal(fastener.material, fastenerMat, "fail-soft does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fail-soft fastenerMesh stays named");
  assert.equal(materialUniformsGroupsAbsent(lidMesh.material), true, "fail-soft lidMesh uniformsGroups is deleted");
  assert.equal(lidMesh.name, "lidMesh", "fail-soft authored lidMesh name stays");
  assert.equal(materialUniformsGroupsAbsent(latchMesh.material), true, "fail-soft latchMesh own undefined is deleted");
  assert.equal(latchMesh.name, "latchMesh", "fail-soft latchMesh name stays");
  assert.equal(mapped.material.uniformsGroups, null, "fail-soft mapped uniformsGroups stays authored");
  assert.equal(sharedGeoVisual.material.uniformsGroups, sharedGeoGroups, "fail-soft geometry shared with a mapped mesh keeps uniformsGroups");
  assert.equal(sharedGeoMat.version, 6, "fail-soft shared material.version stays");
  assert.equal(shader.uniformsGroups, authoredShader, "fail-soft ShaderMaterial uniformsGroups stays authored");
  assert.equal(Object.hasOwn(shader, "uniformsGroups"), true, "fail-soft ShaderMaterial own property stays");
  assert.deepEqual(raw.uniformsGroups, [], "fail-soft RawShaderMaterial uniformsGroups stays authored");
  assert.equal(Object.hasOwn(raw, "uniformsGroups"), true, "fail-soft RawShaderMaterial own property stays");
  assert.deepEqual(colliderGrab.material.uniformsGroups, {}, "fail-soft collider uniformsGroups stays");
  assert.equal(colliderGrab.name, "collider_grab", "fail-soft collider name stays");
});

function materialVertexShaderAbsent(material) {
  return material?.vertexShader === undefined && Object.hasOwn(material, "vertexShader") === false;
}

test("packaged ingest deletes leftover Material vertexShader on color-only visuals and the fastener", () => {
  const { root, groups, fastener, lid, latch } = makePackagedFixture();
  const bodyMesh = groups[0][0].children.find((o) => o.isMesh);
  const bodyExtensions = { multiDraw: true };
  bodyMesh.material.extensions = bodyExtensions;
  ...
  const bodyGroups = [shaderStyleUniformsGroup("body")];
  bodyMesh.material.uniformsGroups = bodyGroups;
  const bodyVertex = "void main(){ /* body */ }";
  bodyMesh.material.vertexShader = bodyVertex;
  bodyMesh.material.fragmentShader = "void main(){gl_FragColor=vec4(1.0);}";
  bodyMesh.material.lights = true;
  bodyMesh.material.clipping = true;
  const bodyIndirect = { label: "body-indirect" };
  bodyMesh.geometry.setIndirect(bodyIndirect);
  const bodyMat = bodyMesh.material;
  bodyMat.version = 4;
  fastener.material = new THREE.MeshBasicMaterial({ color: 0xbe7e31 });
  fastener.material.vertexShader = "";
  const fastenerMat = fastener.material;
  const fastenerIndirect = { label: "fastener-indirect" };
  fastener.geometry.setIndirect(fastenerIndirect);
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  const mappedVertex = "void main(){ /* mapped */ }";
  mapped.material.vertexShader = mappedVertex;
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  sharedGeoMat.vertexShader = "";
  const sharedGeoVisual = new THREE.Mesh(mapped.geometry, sharedGeoMat);
  sharedGeoVisual.name = "sharedGeoBody";
  const interleavedGeo = new THREE.BufferGeometry();
  const interleavedBuffer = new THREE.InterleavedBuffer(new Float32Array([0, 0, 0, 1, 0, 0]), 3);
  interleavedGeo.setAttribute("position", new THREE.InterleavedBufferAttribute(interleavedBuffer, 3, 0));
  const interleavedMat = new THREE.MeshBasicMaterial({ color: 0x633318 });
  const interleavedVertex = "void main(){ /* interleaved */ }";
  interleavedMat.vertexShader = interleavedVertex;
  const interleaved = new THREE.Mesh(interleavedGeo, interleavedMat);
  interleaved.name = "interleavedMesh";
  const shader = new THREE.ShaderMaterial();
  const authoredShader = "void main(){ /* shader */ }";
  shader.vertexShader = authoredShader;
  const shaderMesh = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.1, 0.1), shader);
  shaderMesh.name = "shaderLike";
  const raw = new THREE.RawShaderMaterial();
  const authoredRaw = "void main(){ /* raw */ }";
  raw.vertexShader = authoredRaw;
  const rawMesh = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.1, 0.1), raw);
  rawMesh.name = "rawLike";
  const lit = boxMesh("litMesh", new THREE.MeshStandardMaterial());
  const litVertex = "void main(){ /* lit */ }";
  lit.material.vertexShader = litVertex;
  groups[0][0].add(mapped, interleaved, shaderMesh, rawMesh, lit);
  root.add(sharedGeoVisual);
  const colliderGrab = root.getObjectByName("collider_grab");
  const colliderVertex = "void main(){ /* collider */ }";
  colliderGrab.material.vertexShader = colliderVertex;
  const lidMesh = lid.getObjectByName("lidL0");
  const latchMesh = latch.getObjectByName("latchL0");
  lidMesh.material.vertexShader = "void main(){ /* lid */ }";
  latchMesh.material.vertexShader = null;
  lidMesh.name = "lidMesh";
  latchMesh.name = "latchMesh";
  ...
  assert.equal(bodyMesh.material, bodyMat, "ingest does not replace the body material");
  assert.equal(bodyMesh.material.isMeshBasicMaterial, true, "body stays MeshBasic");
  assert.equal(materialVertexShaderAbsent(bodyMesh.material), true, "in-place color-only body vertexShader is deleted");
  assert.equal(bodyVertex.includes("body"), true, "pin does not mutate the detached string");
  assert.match(bodyMesh.material.fragmentShader, /gl_FragColor/, "vertexShader pin does not touch fragmentShader");
  assert.equal(bodyMesh.material.lights, true, "vertexShader pin does not touch lights");
  assert.equal(bodyMesh.material.clipping, true, "vertexShader pin does not touch clipping");
  assert.equal(materialUniformsGroupsAbsent(bodyMesh.material), true, "uniformsGroups pin still clears leftover uniformsGroups");
  assert.equal(bodyGroups[0].name, "body", "pin does not mutate the detached group");
  assert.equal(materialUniformsNeedUpdateAbsent(bodyMesh.material), true, "uniformsNeedUpdate pin still clears leftover uniformsNeedUpdate");
  assert.equal(materialUniformsAbsent(bodyMesh.material), true, "uniforms pin still clears leftover uniforms");
  assert.equal(bodyUniforms.diffuse.value.getHex(), 0x633318, "pin does not mutate the detached uniforms Color");
  ...
  assert.equal(materialExtensionsAbsent(bodyMesh.material), true, "extensions pin still clears leftover extensions");
  assert.equal(bodyExtensions.multiDraw, true, "pin does not mutate the detached extensions object");
  assert.equal(bodyMesh.geometry.indirect, null, "indirect pin still runs before the vertexShader pin");
  assert.equal(bodyMesh.material.version, 0, "material.version pin still runs");
  assert.equal(unusedChannelAttributesAbsent(bodyMesh.geometry), true, "unused-channel strip still runs");
  assert.equal(materialVertexShaderAbsent(fastener.material), true, "fastener vertexShader is deleted");
  assert.equal(fastener.geometry.indirect, null, "fastener indirect is still cleared");
  assert.equal(fastener.material, fastenerMat, "ingest does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fastenerMesh stays named");
  assert.equal(materialVertexShaderAbsent(lidMesh.material), true, "reserved lidMesh vertexShader is deleted");
  assert.equal(materialVertexShaderAbsent(latchMesh.material), true, "reserved latchMesh vertexShader is deleted");
  assert.equal(lidMesh.name, "lidMesh", "reserved lidMesh name stays");
  assert.equal(latchMesh.name, "latchMesh", "reserved latchMesh name stays");
  assert.equal(mapped.material.vertexShader, mappedVertex, "mapped vertexShader stays authored");
  assert.equal(sharedGeoVisual.material.vertexShader, "", "geometry shared with a mapped mesh keeps vertexShader");
  assert.equal(Object.hasOwn(sharedGeoMat, "vertexShader"), true, "shared-geometry '' stays an own property");
  assert.equal(sharedGeoVisual.name, "sharedGeoBody", "shared-geometry mesh name stays");
  assert.equal(interleaved.material.vertexShader, interleavedVertex, "interleaved vertexShader stays authored");
  assert.equal(shader.vertexShader, authoredShader, "ShaderMaterial authored vertexShader stays");
  assert.equal(Object.hasOwn(shader, "vertexShader"), true, "ShaderMaterial own property stays");
  assert.equal(raw.vertexShader, authoredRaw, "RawShaderMaterial authored vertexShader stays");
  assert.equal(raw.isRawShaderMaterial, true, "RawShaderMaterial stays a RawShaderMaterial");
  assert.equal(lit.material.vertexShader, litVertex, "MeshStandard vertexShader stays authored");
  assert.equal(colliderGrab.material.vertexShader, colliderVertex, "collider vertexShader stays authored");
  assert.equal(Object.hasOwn(colliderGrab.material, "vertexShader"), true, "collider own vertexShader stays");
  assert.equal(colliderGrab.name, "collider_grab", "collider mesh name stays");
});

test("packaged ingest without lod groups still deletes leftover Material vertexShader", () => {
  const { root, body, lid, latch, fastener } = makePackagedFixture({ withLod: false });
  const bodyMesh = visualMeshes(body)[0];
  bodyMesh.material.vertexShader = "void main(){ /* fail-soft */ }";
  bodyMesh.material.fragmentShader = "void main(){gl_FragColor=vec4(1.0);}";
  bodyMesh.material.uniformsGroups = [shaderStyleUniformsGroup("fail-soft")];
  bodyMesh.material.uniformsNeedUpdate = true;
  bodyMesh.material.uniforms = shaderStyleUniformMap();
  ...
  const alphaMesh = boxMesh("alphaBody", new THREE.MeshBasicMaterial({ color: 0x633318 }));
  const sharedAlpha = alphaMesh.material;
  alphaMesh.material.vertexShader = "";
  const alphaTwin = boxMesh("alphaTwin", sharedAlpha);
  body.add(alphaMesh, alphaTwin);
  fastener.material.vertexShader = "void main(){ /* fastener */ }";
  const fastenerMat = fastener.material;
  const mapped = boxMesh("mappedHero", new THREE.MeshBasicMaterial({ color: 0xffffff, map: { isTexture: true } }));
  mapped.material.vertexShader = null;
  body.add(mapped);
  const sharedGeoMat = new THREE.MeshBasicMaterial({ color: 0xc1c3c9 });
  sharedGeoMat.version = 6;
  const sharedGeoVertex = "void main(){ /* shared-geo */ }";
  sharedGeoMat.vertexShader = sharedGeoVertex;
  const sharedGeoVisual = new THREE.Mesh(mapped.geometry, sharedGeoMat);
  sharedGeoVisual.name = "sharedGeoBody";
  body.add(sharedGeoVisual);
  const shader = new THREE.ShaderMaterial();
  ...
  body.add(rawMesh);
  const colliderGrab = root.getObjectByName("collider_grab");
  colliderGrab.material.vertexShader = "void main(){ /* collider */ }";
  const lidMesh = lid.getObjectByName("lidMesh");
  const latchMesh = latch.getObjectByName("latchMesh");
  lidMesh.material.vertexShader = "void main(){ /* lid */ }";
  latchMesh.material.vertexShader = undefined;

  ingestPackagedRoot(root, sidecar);

  assert.equal(root.userData.lod, undefined, "fail-soft still does not invent lod groups");
  assert.equal(bodyMesh.material, bodyMat, "fail-soft does not replace the body material");
  assert.equal(materialVertexShaderAbsent(bodyMesh.material), true, "fail-soft body vertexShader is deleted");
  assert.match(bodyMesh.material.fragmentShader, /gl_FragColor/, "fail-soft vertexShader pin does not touch fragmentShader");
  assert.equal(materialUniformsGroupsAbsent(bodyMesh.material), true, "fail-soft uniformsGroups pin still runs");
  assert.equal(materialUniformsNeedUpdateAbsent(bodyMesh.material), true, "fail-soft uniformsNeedUpdate pin still runs");
  assert.equal(materialUniformsAbsent(bodyMesh.material), true, "fail-soft uniforms pin still runs");
  assert.equal(materialDefaultAttributeValuesAbsent(bodyMesh.material), true, "fail-soft defaultAttributeValues pin still runs");
  assert.equal(materialIndex0AttributeNameAbsent(bodyMesh.material), true, "fail-soft index0AttributeName pin still runs");
  ...
  assert.equal(bodyMesh.matrixWorldNeedsUpdate, false, "fail-soft matrixWorldNeedsUpdate stays false");
  assert.equal(bodyMesh.material.version, 0, "fail-soft material.version pin still runs");
  assert.equal(materialVertexShaderAbsent(alphaMesh.material), true, "fail-soft shared color-only vertexShader is deleted once");
  assert.equal(alphaTwin.material, sharedAlpha, "fail-soft second mesh keeps the shared material");
  assert.equal(materialVertexShaderAbsent(fastener.material), true, "fail-soft fastener vertexShader is deleted");
  assert.equal(fastener.material, fastenerMat, "fail-soft does not replace the fastener material");
  assert.equal(fastener.name, "fastenerMesh", "fail-soft fastenerMesh stays named");
  assert.equal(materialVertexShaderAbsent(lidMesh.material), true, "fail-soft lidMesh vertexShader is deleted");
  assert.equal(lidMesh.name, "lidMesh", "fail-soft authored lidMesh name stays");
  assert.equal(materialVertexShaderAbsent(latchMesh.material), true, "fail-soft latchMesh own undefined is deleted");
  assert.equal(latchMesh.name, "latchMesh", "fail-soft latchMesh name stays");
  assert.equal(mapped.material.vertexShader, null, "fail-soft mapped vertexShader stays authored");
  assert.equal(sharedGeoVisual.material.vertexShader, sharedGeoVertex, "fail-soft geometry shared with a mapped mesh keeps vertexShader");
  assert.equal(sharedGeoMat.version, 6, "fail-soft shared material.version stays");
  assert.equal(shader.vertexShader, authoredShader, "fail-soft ShaderMaterial vertexShader stays authored");
  assert.equal(Object.hasOwn(shader, "vertexShader"), true, "fail-soft ShaderMaterial own property stays");
  assert.equal(raw.vertexShader, "", "fail-soft RawShaderMaterial vertexShader stays authored");
  assert.equal(Object.hasOwn(raw, "vertexShader"), true, "fail-soft RawShaderMaterial own property stays");
  assert.equal(colliderGrab.material.vertexShader.includes("collider"), true, "fail-soft collider vertexShader stays");
  assert.equal(colliderGrab.name, "collider_grab", "fail-soft collider name stays");
});
